...
type DeleteDriveFileReq struct {
	Type      string `query:"type" json:"-"`      // 云文档类型, 示例值："file", 可选值有: `file`：box开头云文档类型, `docx`：docx文档类型, `bitable`：多维表格类型, `folder`：文件夹类型(新版云空间下可用), `doc`：doc文档类型, `sheet`：电子表格类型, `mindnote`：思维笔记类型, `shortcut`：快捷方式类型(新版云空间下可用)
