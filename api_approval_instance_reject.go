...
type RejectApprovalInstanceReq struct {
	...
	Form         ApprovalWidgetList `json:"form,omitempty"`          // 如果审批定义的流程设计中添加了条件分支, 则需要传入条件分支所需的控件数据（JSON 数组）, 否则会影响后续的分支条件流转。说明: 传值时需要将 JSON 序列化为字符串。参数示例值未进行转义, 正确的传值示例可参见下文 请求体示例。示例值: "[{\"id\":\"user_name\", \"type\": \"input\", \"value\":\"test\"}]"
}
