...
// CreateBitableRecord 在多维表格数据表中新增一条记录。
//
// ## 前提条件
...
type CreateBitableRecordReq struct {
	...
	Fields                 map[string]interface{} `json:"fields,omitempty"`                   // 要新增的记录的数据。你需先指定数据表中的字段（即指定列）, 再传入正确格式的数据作为一条记录。注意: 该接口支持的字段类型及其描述如下所示: 文本: 填写字符串格式的值- 数字: 填写数字格式的值- 单选: 填写选项值, 对于新的选项值, 将会创建一个新的选项- 多选: 填写多个选项值, 对于新的选项值, 将会创建一个新的选项。如果填写多个相同的新选项值, 将会创建多个相同的选项- 日期: 填写毫秒级时间戳- 复选框: 填写 true 或 false- 条码- 人员: 填写用户的[open_id](https://open.feishu.cn/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-openid)、[union_id](https://open.feishu.cn/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-union-id) 或 [user_id](https://open.feishu.cn/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-user-id), 类型需要与 user_id_type 指定的类型一致- 电话号码: 填写文本内容- 超链接: 参考以下示例, text 为文本值, link 为 URL 链接- 附件: 填写附件 token, 需要先调用[上传素材](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/media/upload_all)或[分片上传素材](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/media/upload_prepare)接口将附件上传至该多维表格中- 单向关联: 填写被关联表的记录 ID- 双向关联: 填写被关联表的记录 ID- 地理位置: 填写经纬度坐标不同类型字段的数据结构请参考[多维表格记录数据结构](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/app-table-record/bitable-record-data-structure-overview)。示例值: {    "人员": [        {            "id": "ou_2910013f1e6456f16a0ce75ede9abcef"        }    ]}
}

// CreateBitableRecordResp ...
type CreateBitableRecordResp struct {
	Record *CreateBitableRecordRespRecord `json:"record,omitempty"` // 新增记录的内容
}

// CreateBitableRecordRespRecord ...
type CreateBitableRecordRespRecord struct {
	Fields           map[string]interface{}                       `json:"fields,omitempty"`             // 成功新增的记录的数据
	RecordID         string                                       `json:"record_id,omitempty"`          // 新增记录的 ID
	CreatedBy        *CreateBitableRecordRespRecordCreatedBy      `json:"created_by,omitempty"`         // 该记录的创建人信息。本接口不返回该参数
	CreatedTime      int64                                        `json:"created_time,omitempty"`       // 该记录的创建时间。本接口不返回该参数
	LastModifiedBy   *CreateBitableRecordRespRecordLastModifiedBy `json:"last_modified_by,omitempty"`   // 该记录最近一次更新的修改人。本接口不返回该参数
...
type CreateBitableRecordRespRecordCreatedBy struct {
	ID        string `json:"id,omitempty"`         // 创建人的用户 ID, ID 类型与 `user_id_type` 所指定的类型一致
...
type CreateBitableRecordRespRecordLastModifiedBy struct {
	ID        string `json:"id,omitempty"`         // 修改人的用户 ID, ID 类型与 `user_id_type` 所指定的类型一致
	Name      string `json:"name,omitempty"`       // 用户的中文名称
	EnName    string `json:"en_name,omitempty"`    // 用户的英文名称
	Email     string `json:"email,omitempty"`      // 用户的邮箱
	AvatarURL string `json:"avatar_url,omitempty"` // 头像链接字段权限要求（满足任一）: 获取用户基本信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
