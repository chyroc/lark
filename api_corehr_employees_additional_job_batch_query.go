...
DepartmentID string `json:"department_id,omitempty"` // 部门 ID, 可通过[【批量查询部门】](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/corehr-v2/department/batch_get)获取详细信息- 类型与 department_id_type 一致
JobID string `json:"job_id,omitempty"` // 职务 ID, 可通过[【查询单个职务】](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/job/get)获取详细信息字段权限要求（满足任一）: 获取兼职的职务信息读写兼职的职务信息
JobLevelID string `json:"job_level_id,omitempty"` // 职级 ID, 可通过[【查询单个职级】](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/job_level/get)获取详细信息字段权限要求（满足任一）: 获取兼职的职级信息读写兼职的职级信息
...
PositionID string `json:"position_id,omitempty"` // 岗位 ID- 功能灰度中, 如有需求请联系[技术支持](https://applink.feishu.cn/TLJpeNdW)字段权限要求（满足任一）: 读取兼职的岗位读写兼职的岗位
