...
func (r *ChatAPI) GetMember(ctx context.Context, request *GetMemberReq) (*GetMemberResp, *Response, error) {
	req := &requestParam{
		Method:                "GET",
	...
	if err != nil {
		...
		return nil, response, newError("Chat", "GetMember", resp.Code, resp.Msg)
	}
...
type getMemberResp struct {
	...
	Data *GetMemberResp `json:"data,omitempty"` // \-
}

type GetMemberResp struct {
	Items     []*GetMemberRespItem `json:"items,omitempty"`      // member 列表
	PageToken string               `json:"page_token,omitempty"` // 分页标记，当 has_more 为 true 时，会同时返回新的 page_token，否则不返回 page_token
	HasMore   bool                 `json:"has_more,omitempty"`   // 是否还有更多项
}

type GetMemberRespItem struct {
