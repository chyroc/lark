...
type GetVCReserveRespReserveMeetingSettings struct {
	...
	MeetingInitialType int64                                                     `json:"meeting_initial_type,omitempty"` // 会议初始类型可选值有: 多人会议1v1呼叫
...
type GetVCReserveRespReserveMeetingSettingsActionPermission struct {
	Permission         int64                                                                      `json:"permission,omitempty"`          // 权限项可选值有: 是否能成为主持人是否能邀请参会人是否能加入会议
...
type GetVCReserveRespReserveMeetingSettingsActionPermissionPermissionChecker struct {
	CheckField int64    `json:"check_field,omitempty"` // 检查字段类型可选值有: 用户ID（check_list填入用户ID）用户类型（check_list可选值有 "1": 飞书用户、"2": rooms用户、"6": pstn用户、"7": sip用户）租户ID（check_list填入租户tenant_key）
	CheckMode  int64    `json:"check_mode,omitempty"`  // 检查方式可选值有: 在check_list中为有权限（白名单）不在check_list中为有权限（黑名单）
...
type GetVCReserveRespReserveMeetingSettingsAssignHost struct {
	UserType int64  `json:"user_type,omitempty"` // 用户类型, 仅支持设置同租户下的 Lark 用户可选值有: 飞书用户
...
type GetVCReserveRespReserveMeetingSettingsCallSettingCallee struct {
	ID          string                                                              `json:"id,omitempty"`            // 用户ID
	UserType    int64                                                               `json:"user_type,omitempty"`     // 用户类型, 当前仅支持用户类型6(pstn用户)可选值有: 飞书用户rooms用户文档用户neo单品用户neo单品游客用户pstn用户sip用户
