...
// new doc: https://open.feishu.cn/document/directory-v1/employee/resurrect
...
// CreateDirectoryEmployeeResurrectReqEmployeeOrderInDepartment ...
type CreateDirectoryEmployeeResurrectReqEmployeeOrderInDepartment struct {
	DepartmentID               *string `json:"department_id,omitempty"`                 // 部门id, 与department_id_type类型保持一致示例值: "easdiedqds"
	OrderWeightInDeparment     *string `json:"order_weight_in_deparment,omitempty"`     // 用户在部门内的排序权重 长度范围: 1～ 3示例值: "100"
	OrderWeightAmongDeparments *string `json:"order_weight_among_deparments,omitempty"` // 用户多个部门间的排序权重 长度范围: 1～ 3示例值: "20"
	IsMainDepartment           *bool   `json:"is_main_department,omitempty"`            // 是否为用户的主部门（用户只能有一个主部门, 且排序权重应最大, 不填则默认使用排序第一的部门作为主部门)示例值: true
}
