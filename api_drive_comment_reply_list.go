...
type GetDriveCommentReplyListReq struct {
	...
	CommentID  string   `path:"comment_id" json:"-"`    // 评论 ID示例值: "1654857036541812356"
	PageSize   *int64   `query:"page_size" json:"-"`    // 分页大小示例值: 10 最大值: `1 ～ 100`
	PageToken  *string  `query:"page_token" json:"-"`   // 分页标记, 第一次请求不填, 表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token, 下次遍历可采用该 page_token 获取查询结果示例值: 1654857036541812356
	FileType   FileType `query:"file_type" json:"-"`    // 云文档类型示例值: docx可选值有: 旧版文档, 已不推荐使用新版文档类型电子表格类型文件夹类型幻灯片
