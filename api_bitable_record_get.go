...
type GetBitableRecordReq struct {
	AppToken          string  `path:"app_token" json:"-"`            // base app token示例值: "bascnCMII2ORej2RItqpZZUNMIe"
	TableID           string  `path:"table_id" json:"-"`             // table id示例值: "tblxI2tWaxP5dG7p"
	RecordID          string  `path:"record_id" json:"-"`            // 单条记录的 id示例值: "recn0hoyXL"
	TextFieldAsArray  *bool   `query:"text_field_as_array" json:"-"` // 多行文本字段数据是否以数组形式返回。true 表示以数组形式返回。默认为 false示例值: true
	...
	DisplayFormulaRef *bool   `query:"display_formula_ref" json:"-"` // 控制公式、查找引用是否显示完整原样的返回结果。默认为 false示例值: true
	WithSharedURL     *bool   `query:"with_shared_url" json:"-"`     // 控制是否返回该记录的链接, 即 record_url 参数。默认为 false, 即不返回示例值: true
	AutomaticFields   *bool   `query:"automatic_fields" json:"-"`    // 控制是否返回自动计算的字段, 例如 `created_by`/`created_time`/`last_modified_by`/`last_modified_time`, true 表示返回。默认为 false示例值: true
}
