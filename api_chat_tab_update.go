...
// UpdateChatTab 更新指定的会话标签页信息, 包括名称、类型以及内容等。仅支持更新文档类型（doc）或 URL （url）类型的标签页。
//
// ## 前提条件
...
type UpdateChatTabReqChatTab struct {
	TabID      *string                            `json:"tab_id,omitempty"`      // 会话标签页 ID, ID 可以在[添加会话标签页](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-tab/create)或者[拉取会话标签页](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-tab/list_tabs)接口返回值中获取。示例值: "7101214603622940671"
	TabName    *string                            `json:"tab_name,omitempty"`    // 会话标签页名称注意: 会话标签页的名称不能超过 60 个字符示例值: "文档"
	TabType    string                             `json:"tab_type,omitempty"`    // 会话标签页类型注意: 只支持更新 doc、url 类型的标签页, 其他字段为只读字段示例值: "doc"可选值有: 消息类型云文档列表文档Pin会议纪要群公告URL文件合并类型, 包含文件、Doc文档、URL链接合并类型, 包含图片、视频任务
	TabContent *UpdateChatTabReqChatTabTabContent `json:"tab_content,omitempty"` // 会话标签页内容
	TabConfig  *UpdateChatTabReqChatTabTabConfig  `json:"tab_config,omitempty"`  // 会话标签页的配置注意: 仅当  tab_type 取值为 url 时, 该参数生效
}
...
type UpdateChatTabReqChatTabTabContent struct {
	...
	MeetingMinute *string `json:"meeting_minute,omitempty"` // 会议纪要, 因不支持更新 meeting_minute 类型的会话标签页, 该字段为只读字段, 无需传值。示例值: "https://example.feishu.cn/docs/doccnvIXbV22i6hSD3utar4123dx"
	Task          *string `json:"task,omitempty"`           // 任务, 因不支持更新 task 类型的会话标签页, 该字段为只读字段, 无需传值。示例值: "https://bytedance.feishu.cn/client/todo/task_list?guid=fa03fb6d-344b-47d9-83e3-049e3b3da931"
}

// UpdateChatTabResp ...
type UpdateChatTabResp struct {
	ChatTabs []*UpdateChatTabRespChatTab `json:"chat_tabs,omitempty"` // 会话标签页
}
...
type UpdateChatTabRespChatTab struct {
	...
	TabContent *UpdateChatTabRespChatTabTabContent `json:"tab_content,omitempty"` // 会话标签页内容
	TabConfig  *UpdateChatTabRespChatTabTabConfig  `json:"tab_config,omitempty"`  // 会话标签页配置
}
