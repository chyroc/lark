...
type SendHelpdeskMessageReq struct {
	...
	MsgType     MsgType `json:"msg_type,omitempty"`     // 消息类型示例值: "post"可选值有: 普通文本富文本图片卡片消息
	...
	ReceiverID  string  `json:"receiver_id,omitempty"`  // 接收消息用户id示例值: "ou_7346484524"
	ReceiveType *string `json:"receive_type,omitempty"` // 接收消息方式, chat(服务台专属服务群)或user(服务台机器人私聊)。若选择专属服务群, 用户有正在处理的工单将会发送失败。默认以chat方式发送。示例值: "chat"可选值有: 通过服务台专属群发送通过服务台机器人私聊发送
}
