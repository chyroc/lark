...
type GetMessageListRespItemBody struct {
	Content string `json:"content,omitempty"` // 消息内容, JSON 结构序列化后的字符串, 不同消息类型（`msg_type`）对应不同内容。注意: 卡片消息内容与在卡片搭建工具中获取的卡片 JSON 不一致, 暂不支持返回原始卡片 JSON
}

// GetMessageListRespItemMention ...
type GetMessageListRespItemMention struct {
	Key       string `json:"key,omitempty"`        // 被 @ 的用户或机器人序号。例如, 第 3 个被 @ 到的成员, 取值为 `@_user_3`。
	ID        string `json:"id,omitempty"`         // 被 @ 的用户或机器人的 open_id。
	IDType    IDType `json:"id_type,omitempty"`    // 被 @ 的用户或机器人的 ID 类型, 目前仅支持 `open_id` (了解[什么是 Open ID](https://open.feishu.cn/document/home/user-identity-introduction/open-id))。
...
type GetMessageListRespItemSender struct {
