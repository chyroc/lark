...
type StartHelpdeskServiceReq struct {
	...
	AppointedAgents []string `json:"appointed_agents,omitempty"` // 客服 open ids (获取方式参考[获取单个用户信息](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/get)), human_service需要为true示例值: ["ou_7dab8a3d3cdcc9da365777c7ad535d62"]
	...
	CustomizedInfo  *string  `json:"customized_info,omitempty"`  // 工单来源自定义信息, 长度限制1024字符, 如设置, [获取工单详情](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/helpdesk-v1/ticket/get)会返回此信息示例值: "测试自定义字段信息"
}

// StartHelpdeskServiceResp ...
type StartHelpdeskServiceResp struct {
	ChatID   string `json:"chat_id,omitempty"`   // 客服群open ID
	TicketID string `json:"ticket_id,omitempty"` // 创建的工单 ID（仅人工工单返回该参数）
}
