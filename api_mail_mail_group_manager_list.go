...
type GetMailGroupManagerListReq struct {
	...
	PageSize    *int64  `query:"page_size" json:"-"`    // 分页大小示例值: 20默认值: `20` 取值范围: `0` ～ `200
	PageToken   *string `query:"page_token" json:"-"`   // 分页标记, 第一次请求不填, 表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token, 下次遍历可采用该 page_token 获取查询结果示例值: xxxxxx 长度范围: `0` ～ `1048576` 字符
