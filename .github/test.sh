...
GO111MODULE=on echo 'mode: atomic' > coverage.txt && \
  go list ./... | \
    xargs -n1 -I{} sh -c 'LOCAL_TEST=true go test -v -covermode=atomic -coverprofile=coverage.tmp -coverpkg=./... -parallel 1 -p 1 -count=1 -gcflags=-l {} && tail -n +2 coverage.tmp >> coverage.txt' && \
  rm coverage.tmp
