...
func (r *ChatAPI) GetChatListBySearch(ctx context.Context, request *GetChatListBySearchReq) (*GetChatListBySearchResp, *Response, error) {
	req := &requestParam{
		Method:                "GET",
	...
	resp := new(getChatListBySearchResp)

	response, err := r.cli.request(ctx, req, resp)
	if err != nil {
		return nil, response, err
	} else if resp.Code != 0 {
		return nil, response, newError("Chat", "GetChatListBySearch", resp.Code, resp.Msg)
	}

	return resp.Data, response, nil
}

type GetChatListBySearchReq struct {
...
type getChatListBySearchResp struct {
	...
	Data *GetChatListBySearchResp `json:"data,omitempty"` // \-
}
