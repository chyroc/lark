...
// GenerateCaldavConf 调用该接口为当前用户生成一个 CalDAV 账号密码, 用于将飞书日历信息同步到本地设备日历。
//
...
type GenerateCaldavConfReq struct {
	DeviceName *string `json:"device_name,omitempty"` // 需要同步日历的设备名, 在日历中用来展示。默认值: 空示例值: "iPhone" 最大长度: `100` 字符
}

// GenerateCaldavConfResp ...
type GenerateCaldavConfResp struct {
	Password      string `json:"password,omitempty"`       // CalDAV 密码。
	UserName      string `json:"user_name,omitempty"`      // CalDAV 用户名。
	ServerAddress string `json:"server_address,omitempty"` // 服务器地址
	DeviceName    string `json:"device_name,omitempty"`    // 设备名。与你发送请求时传入的设备名一致。
}
