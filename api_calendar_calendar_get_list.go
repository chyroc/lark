...
func (r *CalendarService) GetCalendarList(ctx context.Context, request *GetCalendarListReq, options ...MethodOptionFunc) (*GetCalendarListResp, *Response, error) {
	...
	r.cli.log(ctx, LogLevelInfo, "[lark] Calendar#GetCalendarList call api")
	r.cli.log(ctx, LogLevelDebug, "[lark] Calendar#GetCalendarList request: %s", jsonString(request))
	...
	if err != nil {
		r.cli.log(ctx, LogLevelError, "[lark] Calendar#GetCalendarList GET https://open.feishu.cn/open-apis/calendar/v4/calendars failed, request_id: %s, status_code: %d, error: %s", requestID, statusCode, err)
		return nil, response, err
	} else if resp.Code != 0 {
		r.cli.log(ctx, LogLevelError, "[lark] Calendar#GetCalendarList GET https://open.feishu.cn/open-apis/calendar/v4/calendars failed, request_id: %s, status_code: %d, code: %d, msg: %s", requestID, statusCode, resp.Code, resp.Msg)
		return nil, response, NewError("Calendar", "GetCalendarList", resp.Code, resp.Msg)
	}

	r.cli.log(ctx, LogLevelDebug, "[lark] Calendar#GetCalendarList success, request_id: %s, status_code: %d, response: %s", requestID, statusCode, jsonString(resp.Data))
