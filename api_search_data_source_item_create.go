...
type CreateSearchDataSourceItemReq struct {
	...
	ID             string                                 `json:"id,omitempty"`              // item 在 datasource 中的唯一标识, 示例值: "01010111", 最大长度: `50` 字符
	...
	FieldACL       *string                                `json:"field_acl,omitempty"`       // $$$search.v2.type.item.prop.field_acl.desc$$$, 示例值: "$$$search.v2.type.item.prop.field_acl.string.example$$$"
}
...
type CreateSearchDataSourceItemReqACL struct {
	...
	Type   *string `json:"type,omitempty"`   // 权限值类型, 示例值: "user", 可选值有: `user`: 访问权限控制中指定“用户”可以访问或拒绝访问该条数据, `group`: (已下线)访问权限控制中指定“用户组”可以访问或拒绝访问该条数据
}
