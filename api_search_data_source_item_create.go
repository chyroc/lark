...
type CreateSearchDataSourceItemReq struct {
	...
	ID             string                                 `json:"id,omitempty"`              // item 在 datasource 中的唯一标识, 只允许英文字母、数字和下划线, 示例值: "my_item_01010111", 最大长度: `128` 字符
	...
	StructuredData string                                 `json:"structured_data,omitempty"` // 结构化数据（以 json 字符串传递）, 这些字段是搜索结果的展示字段(特殊字段无须在此另外指定);具体格式可参参考 [接入指南](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/search-v2/common-template-intergration-handbook) 请求创建数据项部分。这里的示例遵循了”创建数据范式“部分中的数据范式示例, 请按自己定义的数据范式填写数据, 示例值: "{"description":"问题出现的环境和复现方法描述……", "priority":"HIGH"}"
