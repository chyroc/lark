...
// BatchDeleteHireEcoBackgroundCheckPackage 删除背调帐号下的背调套餐和附加调查项信息。删除操作不会影响已创建的背调。
//
...
type BatchDeleteHireEcoBackgroundCheckPackageReq struct {
	...
	PackageIDList        []string `json:"package_id_list,omitempty"`         // 要删除的套餐 ID 列表。删除套餐不影响已安排的背调示例值: ["7230753910687080001"]
	AdditionalItemIDList []string `json:"additional_item_id_list,omitempty"` // 要删除的附加调查项 ID 列表。删除附加调查项不影响已安排的背调示例值: ["7230753910687080002"]
}
