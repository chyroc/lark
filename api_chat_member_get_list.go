...
func (r *ChatService) GetChatMemberList(ctx context.Context, request *GetChatMemberListReq, options ...MethodOptionFunc) (*GetChatMemberListResp, *Response, error) {
	...
	r.cli.log(ctx, LogLevelInfo, "[lark] Chat#GetChatMemberList call api")
	r.cli.log(ctx, LogLevelDebug, "[lark] Chat#GetChatMemberList request: %s", jsonString(request))
	...
	if err != nil {
		r.cli.log(ctx, LogLevelError, "[lark] Chat#GetChatMemberList GET https://open.feishu.cn/open-apis/im/v1/chats/:chat_id/members failed, request_id: %s, status_code: %d, error: %s", requestID, statusCode, err)
		return nil, response, err
	} else if resp.Code != 0 {
		r.cli.log(ctx, LogLevelError, "[lark] Chat#GetChatMemberList GET https://open.feishu.cn/open-apis/im/v1/chats/:chat_id/members failed, request_id: %s, status_code: %d, code: %d, msg: %s", requestID, statusCode, resp.Code, resp.Msg)
		return nil, response, NewError("Chat", "GetChatMemberList", resp.Code, resp.Msg)
	}

	r.cli.log(ctx, LogLevelDebug, "[lark] Chat#GetChatMemberList success, request_id: %s, status_code: %d, response: %s", requestID, statusCode, jsonString(resp.Data))
