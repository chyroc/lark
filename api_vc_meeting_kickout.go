...
type KickoutVCMeetingReq struct {
	MeetingID    string                            `path:"meeting_id" json:"-"`     // 会议ID示例值: "6911188411932033028"
...
type KickoutVCMeetingReqKickoutUser struct {
	...
	UserType *int64  `json:"user_type,omitempty"` // 用户类型示例值: 1可选值有: 飞书用户rooms用户文档用户neo单品用户neo单品游客用户pstn用户sip用户
}
...
type KickoutVCMeetingRespKickoutResult struct {
	...
	UserType int64  `json:"user_type,omitempty"` // 用户类型可选值有: lark用户rooms用户文档用户neo单品用户neo单品游客用户pstn用户sip用户
	Result   int64  `json:"result,omitempty"`    // 移除结果可选值有: 移除成功移除失败
}
