...
type GetHireOfferListReq struct {
	...
	TalentID           string  `query:"talent_id" json:"-"`             // 人才 ID, 如何获取人才 ID 请参考[获取人才列表](https://open.feishu.cn/document/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/talent/list)示例值: 7096320678581242123
...
type GetHireOfferListRespItem struct {
	ID            string                                `json:"id,omitempty"`             // Offer ID, 详情可参考: [获取 Offer 详情](https://open.feishu.cn/document/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/offer/get)
	JobInfo       *GetHireOfferListRespItemJobInfo      `json:"job_info,omitempty"`       // Offer 职位
	CreateTime    int64                                 `json:"create_time,omitempty"`    // 创建时间戳（单位: 毫秒）
	OfferStatus   int64                                 `json:"offer_status,omitempty"`   // Offer 状态可选值有: 未申请审批中审批已撤回审批通过审批不通过Offer 已发出候选人已接受候选人已拒绝Offer 已失效未审批实习待入职（仅实习 Offer 具有）实习已入职（仅实习 Offer 具有）实习已离职（仅实习 Offer 具有）
	...
	ApplicationID string                                `json:"application_id,omitempty"` // Offer 投递 ID, 获取投递信息请参考[获取投递信息](https://open.feishu.cn/document/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/application/get)
}

// GetHireOfferListRespItemEmployeeType ...
type GetHireOfferListRespItemEmployeeType struct {
	ID     string `json:"id,omitempty"`      // Offer 人员类型 ID, 与入参 `employee_type_id_type` 类型一致
	ZhName string `json:"zh_name,omitempty"` // Offer 人员类型中文名称
	EnName string `json:"en_name,omitempty"` // Offer 人员类型英文名称
}

// GetHireOfferListRespItemJobInfo ...
type GetHireOfferListRespItemJobInfo struct {
	JobID   string `json:"job_id,omitempty"`   // Offer 职位 ID, 详情可参考: [获取职位详情](https://open.feishu.cn/document/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/job/get_detail)
