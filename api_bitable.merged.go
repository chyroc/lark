...
// UpdateBitableMeta 通过 app_token 更新多维表格名称
//
...
// UpdateBitableMetaResp ...
type UpdateBitableMetaResp struct {
	App *UpdateBitableMetaRespApp `json:"app,omitempty"` // 多维表格的名字
}
...
// 注意：
// 1.目前暂不支持字段类型为“双向关联”和“查找引用”的返回。
// 2.对于“查找引用”字段可以转换为“公式”字段，查找引用本质上是 lookup 公式。
//
...
type GetBitableRecordListReq struct {
	...
	TextFieldAsArray *bool   `query:"text_field_as_array" json:"-"` // 控制多行文本字段数据的返回格式，true 表示以数组形式返回。,注意：,1.多行文本中如果有超链接部分，则会返回链接的 URL。,2.目前可以返回多行文本中 URL 类型为多维表格链接以及@人员的数据结构。,3.不支持多行文本中链接是飞书 doc、sheet的URL类型。, 示例值：true
