...
type GetDriveCommentListReq struct {
	FileToken  string   `path:"file_token" json:"-"`    // 云文档的 token。获取方式参考 [如何获取云文档相关 token](https://open.feishu.cn/document/ukTMukTMukTM/uczNzUjL3czM14yN3MTN#08bb5df6)。示例值: "XIHSdYSI7oMEU1xrsnxc8fabcef"
	FileType   FileType `query:"file_type" json:"-"`    // 云文档类型示例值: doc可选值有: 旧版文档类型, 已不推荐使用新版文档类型电子表格类型文件类型幻灯片
	IsWhole    *bool    `query:"is_whole" json:"-"`     // 是否全文评论示例值: false
	IsSolved   *bool    `query:"is_solved" json:"-"`    // 是否已解决（可选）示例值: false
	PageToken  *string  `query:"page_token" json:"-"`   // 分页标记, 第一次请求不填, 表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token, 下次遍历可采用该 page_token 获取查询结果示例值: 7153511712153412356
	PageSize   *int64   `query:"page_size" json:"-"`    // 分页大小, 默认每页返回 50 个评论示例值: 10 最大值: `100
...
type GetDriveCommentListRespItemReplyListReplyExtra struct {
	ImageList []string `json:"image_list,omitempty"` // 评论中的图片 Token list。暂不支持通过接口下载图片。
}
