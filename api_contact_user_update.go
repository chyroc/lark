...
//
// 本接口已为历史版本, 不再维护更新, 不推荐使用。推荐你使用[修改用户部分信息](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/patch)接口。
...
type UpdateUserReq struct {
	...
	DepartmentIDType *DepartmentIDType          `query:"department_id_type" json:"-"` // 此次调用中使用的部门ID的类型示例值: open_department_id可选值有: 以自定义department_id来标识部门以open_department_id来标识部门默认值: `open_department_id`
	Name             string                     `json:"name,omitempty"`               // 用户名示例值: "张三" 最小长度: `1` 字符
	EnName           *string                    `json:"en_name,omitempty"`            // 英文名示例值: "San Zhang"
	Nickname         *string                    `json:"nickname,omitempty"`           // 别名示例值: "Alex Zhang"
	Email            *string                    `json:"email,omitempty"`              // 邮箱注意: 1. 非中国大陆手机号成员必须同时添加邮箱2. 邮箱不可重复示例值: "zhangsan@gmail.com"
	Mobile           string                     `json:"mobile,omitempty"`             // 手机号注意: 1. 在本企业内不可重复2. 未认证企业仅支持添加中国大陆手机号, 通过飞书认证的企业允许添加海外手机号3. 国际电话区号前缀中必须包含加号 +4. 该 mobile 字段在海外版飞书非必填示例值: "13011111111 (其他例子, 中国大陆手机号: 13011111111 或 +8613011111111, 非中国大陆手机号: +41446681800)"
	MobileVisible    *bool                      `json:"mobile_visible,omitempty"`     // 手机号码可见性, true 为可见, false 为不可见, 目前默认为 true。不可见时, 组织员工将无法查看该员工的手机号码示例值: false
	Gender           *int64                     `json:"gender,omitempty"`             // 性别示例值: 1可选值有: 保密男女
	...
	LeaderUserID     *string                    `json:"leader_user_id,omitempty"`     // 用户的直接主管的用户ID, ID值与查询参数中的user_id_type 对应。不同 ID 的说明参见 [用户相关的 ID 概念](https://open.feishu.cn/document/home/user-identity-introduction/introduction)获取方式参见[如何获取不同的用户 ID](https://open.feishu.cn/document/home/user-identity-introduction/open-id)示例值: "ou_7dab8a3d3cdcc9da365777c7ad535d62"
	City             *string                    `json:"city,omitempty"`               // 工作城市示例值: "杭州"
	...
	WorkStation      *string                    `json:"work_station,omitempty"`       // 工位示例值: "北楼-H34"
	JoinTime         *int64                     `json:"join_time,omitempty"`          // 入职时间, 时间戳格式, 表示从1970年1月1日开始所经过的秒数示例值: 2147483647
	EmployeeNo       *string                    `json:"employee_no,omitempty"`        // 工号示例值: "1"
	...
	CustomAttrs      []*UpdateUserReqCustomAttr `json:"custom_attrs,omitempty"`       // 自定义字段, 请确保你的组织管理员已在管理后台/组织架构/成员字段管理/自定义字段管理/全局设置中开启了“允许开放平台 API 调用“, 否则该字段不会生效/返回。更多详情参见[用户接口相关问题](https://open.feishu.cn/document/ugTN1YjL4UTN24CO1UjN/uQzN1YjL0cTN24CN3UjN#77061525)
	EnterpriseEmail  *string                    `json:"enterprise_email,omitempty"`   // 企业邮箱, 请先确保已在管理后台启用飞书邮箱服务创建用户时, 企业邮箱的使用方式参见[用户接口相关问题](https://open.feishu.cn/document/ugTN1YjL4UTN24CO1UjN/uQzN1YjL0cTN24CN3UjN#77061525)示例值: "demo@mail.com"
	JobTitle         *string                    `json:"job_title,omitempty"`          // 职务示例值: "xxxxx"
	IsFrozen         *bool                      `json:"is_frozen,omitempty"`          // 是否暂停用户示例值: false
}

// UpdateUserReqCustomAttr ...
type UpdateUserReqCustomAttr struct {
	Type  *string                       `json:"type,omitempty"`  // 自定义字段类型   - `TEXT`: 文本- `HREF`: 网页- `ENUMERATION`: 枚举- `PICTURE_ENUM`: 图片- `GENERIC_USER`: 用户具体说明参见常见问题的[用户接口相关问题](https://open.feishu.cn/document/ugTN1YjL4UTN24CO1UjN/uQzN1YjL0cTN24CN3UjN#77061525)示例值: "TEXT"
	ID    *string                       `json:"id,omitempty"`    // 自定义字段ID示例值: "DemoId"
...
type UpdateUserReqCustomAttrValue struct {
	Text        *string                                  `json:"text,omitempty"`         // 字段类型为`TEXT`时该参数定义字段值, 必填；字段类型为`HREF`时该参数定义网页标题, 必填示例值: "DemoText"
	URL         *string                                  `json:"url,omitempty"`          // 字段类型为 HREF 时, 该参数定义默认 URL, 例如手机端跳转小程序, PC端跳转网页示例值: "http://www.fs.cn"
	PcURL       *string                                  `json:"pc_url,omitempty"`       // 字段类型为 HREF 时, 该参数定义PC端 URL示例值: "http://www.fs.cn"
	OptionID    *string                                  `json:"option_id,omitempty"`    // 字段类型为 ENUMERATION 或 PICTURE_ENUM 时, 该参数定义选项值示例值: "edcvfrtg"
...
type UpdateUserReqCustomAttrValueGenericUser struct {
	...
	Type int64  `json:"type,omitempty"` // 用户类型: 1: 用户目前固定为1, 表示用户类型示例值: 1
}
...
type UpdateUserReqOrder struct {
	...
	UserOrder       *int64  `json:"user_order,omitempty"`       // 用户在其直属部门内的排序, 数值越大, 排序越靠前示例值: 100
	DepartmentOrder *int64  `json:"department_order,omitempty"` // 用户所属的多个部门间的排序, 数值越大, 排序越靠前示例值: 100
...
type UpdateUserRespUser struct {
	...
	Name            string                          `json:"name,omitempty"`              // 用户名字段权限要求（满足任一）: 获取用户基本信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	EnName          string                          `json:"en_name,omitempty"`           // 英文名字段权限要求（满足任一）: 获取用户基本信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	Nickname        string                          `json:"nickname,omitempty"`          // 别名字段权限要求（满足任一）: 获取用户基本信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	Email           string                          `json:"email,omitempty"`             // 邮箱注意: 1. 非中国大陆手机号成员必须同时添加邮箱2. 邮箱不可重复字段权限要求: 获取用户邮箱信息
	Mobile          string                          `json:"mobile,omitempty"`            // 手机号注意: 1. 在本企业内不可重复2. 未认证企业仅支持添加中国大陆手机号, 通过飞书认证的企业允许添加海外手机号3. 国际电话区号前缀中必须包含加号 +4. 该 mobile 字段在海外版飞书非必填字段权限要求: 获取用户手机号
	MobileVisible   bool                            `json:"mobile_visible,omitempty"`    // 手机号码可见性, true 为可见, false 为不可见, 目前默认为 true。不可见时, 组织员工将无法查看该员工的手机号码
	Gender          int64                           `json:"gender,omitempty"`            // 性别可选值有: 保密男女字段权限要求（满足任一）: 获取用户性别以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	...
	Avatar          *UpdateUserRespUserAvatar       `json:"avatar,omitempty"`            // 用户头像信息字段权限要求（满足任一）: 获取用户基本信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	Status          *UpdateUserRespUserStatus       `json:"status,omitempty"`            // 用户状态, 枚举类型, 包括is_frozen、is_resigned、is_activated、is_exited 。用户状态转移参见: [用户状态图](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/field-overview#4302b5a1)字段权限要求（满足任一）: 获取用户受雇信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	DepartmentIDs   []string                        `json:"department_ids,omitempty"`    // 用户所属部门的ID列表, 一个用户可属于多个部门。ID值的类型与查询参数中的department_id_type 对应。不同 ID 的说明与department_id的获取方式参见 [部门ID说明](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/department/field-overview#23857fe0)字段权限要求（满足任一）: 获取用户组织架构信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	LeaderUserID    string                          `json:"leader_user_id,omitempty"`    // 用户的直接主管的用户ID, ID值与查询参数中的user_id_type 对应。不同 ID 的说明参见 [用户相关的 ID 概念](https://open.feishu.cn/document/home/user-identity-introduction/introduction)获取方式参见[如何获取不同的用户 ID](https://open.feishu.cn/document/home/user-identity-introduction/open-id)字段权限要求（满足任一）: 获取用户组织架构信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	City            string                          `json:"city,omitempty"`              // 工作城市字段权限要求（满足任一）: 获取用户受雇信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	Country         string                          `json:"country,omitempty"`           // 国家或地区Code缩写, 具体写入格式请参考 [国家/地区码表](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/country-code-description)字段权限要求（满足任一）: 获取用户受雇信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	WorkStation     string                          `json:"work_station,omitempty"`      // 工位字段权限要求（满足任一）: 获取用户受雇信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	JoinTime        int64                           `json:"join_time,omitempty"`         // 入职时间, 时间戳格式, 表示从1970年1月1日开始所经过的秒数字段权限要求（满足任一）: 获取用户受雇信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	IsTenantManager bool                            `json:"is_tenant_manager,omitempty"` // 是否是租户超级管理员字段权限要求（满足任一）: 获取用户受雇信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	EmployeeNo      string                          `json:"employee_no,omitempty"`       // 工号字段权限要求（满足任一）: 获取用户受雇信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	EmployeeType    int64                           `json:"employee_type,omitempty"`     // 员工类型, 可选值有: `1`: 正式员工- `2`: 实习生- `3`: 外包- `4`: 劳务- `5`: 顾问   同时可读取到自定义员工类型的 int 值, 可通过下方接口获取到该租户的自定义员工类型的名称, 参见[获取人员类型](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/employee_type_enum/list)字段权限要求（满足任一）: 获取用户受雇信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	Orders          []*UpdateUserRespUserOrder      `json:"orders,omitempty"`            // 用户排序信息。用于标记通讯录下组织架构的人员顺序, 人员可能存在多个部门中, 且有不同的排序。字段权限要求（满足任一）: 获取用户组织架构信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	CustomAttrs     []*UpdateUserRespUserCustomAttr `json:"custom_attrs,omitempty"`      // 自定义字段, 请确保你的组织管理员已在管理后台/组织架构/成员字段管理/自定义字段管理/全局设置中开启了“允许开放平台 API 调用“, 否则该字段不会生效/返回。更多详情参见[用户接口相关问题](https://open.feishu.cn/document/ugTN1YjL4UTN24CO1UjN/uQzN1YjL0cTN24CN3UjN#77061525)字段权限要求（满足任一）: 获取用户受雇信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	EnterpriseEmail string                          `json:"enterprise_email,omitempty"`  // 企业邮箱, 请先确保已在管理后台启用飞书邮箱服务创建用户时, 企业邮箱的使用方式参见[用户接口相关问题](https://open.feishu.cn/document/ugTN1YjL4UTN24CO1UjN/uQzN1YjL0cTN24CN3UjN#77061525)字段权限要求（满足任一）: 获取用户受雇信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	JobTitle        string                          `json:"job_title,omitempty"`         // 职务字段权限要求（满足任一）: 获取用户受雇信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
...
type UpdateUserRespUserCustomAttr struct {
	Type  string                             `json:"type,omitempty"`  // 自定义字段类型   - `TEXT`: 文本- `HREF`: 网页- `ENUMERATION`: 枚举- `PICTURE_ENUM`: 图片- `GENERIC_USER`: 用户具体说明参见常见问题的[用户接口相关问题](https://open.feishu.cn/document/ugTN1YjL4UTN24CO1UjN/uQzN1YjL0cTN24CN3UjN#77061525)
...
type UpdateUserRespUserCustomAttrValueGenericUser struct {
	...
	Type int64  `json:"type,omitempty"` // 用户类型: 1: 用户目前固定为1, 表示用户类型
}
