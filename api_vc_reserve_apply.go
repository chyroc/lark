...
type ApplyVCReserveReq struct {
	...
	EndTime         *string                           `json:"end_time,omitempty"`         // 预约到期时间（unix时间, 单位sec）, 多人会议必填示例值: "1608888867"
	OwnerID         *string                           `json:"owner_id,omitempty"`         // 指定会议归属人, 使用 tenant_access_token 时生效且必传, 指定对象必须为同租户下的合法飞书用户使用 user_access_token 时, 该参数不生效, 设置归属人无意义示例值: "ou_3ec3f6a28a0d08c45d895276e8e5e19b"
...
type ApplyVCReserveReqMeetingSettings struct {
	...
	MeetingInitialType *int64                                              `json:"meeting_initial_type,omitempty"` // 会议初始类型示例值: 1可选值有: 多人会议1v1呼叫(仅支持预约PSTN用户)
