...
type GetApplicationFeedbackListReq struct {
	...
	FeedbackType *int64  `query:"feedback_type" json:"-"` // 反馈类型, 不填写则表示查询所有反馈类型。示例值: 1可选值有: 故障反馈产品建议
	Status       *int64  `query:"status" json:"-"`        // 反馈处理状态, 不填写则表示查询所有处理类型。示例值: 0可选值有: 反馈未处理反馈已处理反馈处理中反馈已关闭
	...
	PageToken    *string `query:"page_token" json:"-"`    // 分页标记, 第一次请求不填, 表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token, 下次遍历可采用该 page_token 获取查询结果示例值: "7064688334618378259"
	PageSize     *int64  `query:"page_size" json:"-"`     // 本次拉取反馈列表最大个数示例值: 100默认值: `100` 取值范围: `1` ～ `100`
}
...
type GetApplicationFeedbackListRespFeedback struct {
	...
	FeedbackType int64    `json:"feedback_type,omitempty"` // 反馈类型可选值有: 故障反馈产品建议
	Status       int64    `json:"status,omitempty"`        // 反馈处理状态可选值有: 反馈未处理反馈已处理反馈处理中反馈已关闭
	...
	Source       int64    `json:"source,omitempty"`        // 反馈来源: 1: 小程序 2: 网页应用 3: 机器人 4: webSDK可选值有: 小程序网页应用机器人WebSDK
	Contact      string   `json:"contact,omitempty"`       // 用户联系方式, 只有用户填写联系方式后返回字段权限要求（满足任一）: 获取用户邮箱信息获取用户手机号
