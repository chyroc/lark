...
type GetChatListOfSelfReq struct {
	...
	SortType   *string `query:"sort_type" json:"-"`    // 群组排序方式示例值: ByCreateTimeAsc可选值有: 按群组创建时间升序排列按群组活跃时间降序排列。因群组活跃时间变动频繁, 使用 `ByActiveTimeDesc` 排序方式可能会造成群组遗漏。例如, 设置分页大小为 10, 发起第一次请求获取到第一页数据后, 原本排在第 11 位的群组中有群成员发送了一条消息, 那么该群组将被排列到第 1 位, 此时发起请求获取第二页数据时, 该群组将不能被获取到, 需要再从第一页开始获取。默认值: `ByCreateTimeAsc`
	...
	PageSize   *int64  `query:"page_size" json:"-"`    // 分页大小, 用来限制一次请求返回的数据条目数。示例值: 10默认值: `20` 最大值: `100`
}

// GetChatListOfSelfResp ...
type GetChatListOfSelfResp struct {
	Items     []*GetChatListOfSelfRespItem `json:"items,omitempty"`      // 群组列表
