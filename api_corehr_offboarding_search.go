...
type SearchCoreHROffboardingReq struct {
	...
	Reasons                      []string `json:"reasons,omitempty"`                         // 离职原因列表, 可以通过[【查询员工离职原因列表】](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/offboarding/query)接口获取, 查询时不返回下级原因相关的离职信息。为空时默认搜索所有离职数据。 <b>字段权限要求: </b> 按照离职原因搜索corehr:employment.offboarding_reason.search:read, 确认已开通该权限。示例值: ["voluntary"]
	EmployeeReasons              []string `json:"employee_reasons,omitempty"`                // 离职原因（员工）列表, 可以通过[【查询员工离职原因列表】](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/offboarding/query)接口获取, 查询时不返回下级原因相关的离职信息。为空时默认搜索所有离职数据。 <b>字段权限要求: </b> 按照离职原因搜索示例值: ["voluntary"]
}
