...
// CreateBaseAppRole 新增多维表格高级权限中自定义的角色。
//
...
if r.cli.mock.mockBaseCreateBaseAppRole != nil {
 ...
 NeedUserAccessToken: true,
	...
	resp := new(createBaseAppRoleResp)
...
type CreateBaseAppRoleReq struct { 
...
RoleName string `json:"role_name,omitempty"` // 自定义权限的名字示例值: "普通用户" 长度范围: `0` ～ `100` 字符
TableRoles []*CreateBaseAppRoleReqTableRole `json:"table_roles,omitempty"` // 针对数据表的权限设置 长度范围: `0` ～ `100`
BlockRoles []*CreateBaseAppRoleReqBlockRole `json:"block_roles,omitempty"` // 针对仪表盘的权限设置 长度范围: `0` ～ `100`
BaseRule interface{} `json:"base_rule,omitempty"` // 多维表格点位的权限。- 未设置时, 表示自定义角色拥有所有点位权限。- 设置时, 可设置以下两种权限: -  `base_complex_edit` : 设置是否可以创建副本、下载、打印多维表格    - `copy`: 设置是否可以复制多维表格内容该参数类型为 map, 其中 key 是权限点位名称, value 是权限开关。value 枚举值有: `0`: 无权限- `1`: 有权限注意: 仅高级权限为 v2 版本的多维表格支持该参数。是否是 v2 版本可调用[获取多维表格元数据](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/app/get)查看。示例值: {"base_complex_edit": 1, "copy": 0}
}
...
// CreateBaseAppRoleReqBlockRole ...
type CreateBaseAppRoleReqBlockRole struct { 
...
BlockPerm int64 `json:"block_perm,omitempty"` // 设置仪表盘的权限示例值: 0可选值有: 无权限可阅读
}
...
// CreateBaseAppRoleReqTableRole ...
type CreateBaseAppRoleReqTableRole struct { 
...
TableName *string `json:"table_name,omitempty"` // 数据表名称（与下方 table_id 至少填写一项）。示例值: "数据表1" 长度范围: `0` ～ `50` 字符
TableID *string `json:"table_id,omitempty"` // 多维表格数据表的唯一标识。（与上方 table_name 至少填写一项）。获取方式: 你可通过多维表格 URL 获取 `table_id`, 下图高亮部分即为当前数据表的 `table_id`- 也可通过[列出数据表](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/app-table/list)接口获取 `table_id`  ![](//sf3-cn.feishucdn.com/obj/open-platform-opendoc/18741fe2a0d3cafafaf9949b263bb57d_yD1wkOrSju.png?height=746&lazyload=true&maxWidth=700&width=2976)示例值: "tblKz5D60T4JlfcT" 长度范围: `0` ～ `50` 字符
RecRule *CreateBaseAppRoleReqTableRoleRecRule `json:"rec_rule,omitempty"` // 记录筛选条件, 当 `table_perm` 为 1 或 2 时生效。用于指定可编辑或可阅读的记录。
OtherRecRule *CreateBaseAppRoleReqTableRoleOtherRecRule `json:"other_rec_rule,omitempty"` // 记录筛选条件, 在 `rec_rule.other_perm` 为 0 时生效。对于未命中 `rec_rule` 的记录, 通过 `other_rec_rule` 指定可阅读记录范围；此时, 既未命中 `rec_rule`、也未命中 `other_rec_rule` 的记录会被禁止阅读。注意: 仅高级权限为 v2 版本的多维表格支持该参数。是否是 v2 版本可调用[获取多维表格元数据](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/app/get)查看。
FieldPerm interface{} `json:"field_perm,omitempty"` // 字段权限, 仅在 `table_perm` 为 1和 2 时生效。用于设置字段可编辑或可阅读。类型为 map, key 是字段名称, value 是字段权限。对于未设置的字段, 默认无权限。value 枚举值有: `1`: 可阅读- `2`: 可添加- `3`: 可编辑示例值: {"姓名": 1, "年龄": 2}
...
ViewPerm *int64 `json:"view_perm,omitempty"` // 设置视图的权限。示例值: 2可选值有: 可阅读可编辑默认值: `2` 取值范围: `0` ～ `2`
...
// CreateBaseAppRoleReqTableRoleOtherRecRule ...
type CreateBaseAppRoleReqTableRoleOtherRecRule struct { 
Conditions []*CreateBaseAppRoleReqTableRoleOtherRecRuleCondition `json:"conditions,omitempty"` // 记录筛选条件, 用于指定可阅读的记录。 长度范围: `0` ～ `10`
...
// CreateBaseAppRoleReqTableRoleOtherRecRuleCondition ...
type CreateBaseAppRoleReqTableRoleOtherRecRuleCondition struct { 
...
// CreateBaseAppRoleReqTableRoleRecRule ...
type CreateBaseAppRoleReqTableRoleRecRule struct { 
Conditions []*CreateBaseAppRoleReqTableRoleRecRuleCondition `json:"conditions,omitempty"` // 记录筛选条件, 用于指定可编辑或可阅读的记录。 长度范围: `0` ～ `10`
Conjunction *string `json:"conjunction,omitempty"` // 多个筛选条件的关系示例值: "and"可选值有: 与或默认值: `and`
...
// CreateBaseAppRoleReqTableRoleRecRuleCondition ...
type CreateBaseAppRoleReqTableRoleRecRuleCondition struct { 
FieldName string `json:"field_name,omitempty"` // 条件字段的名称。记录筛选条件是“创建人包含访问者本人”时, 此参数值为 ""。示例值: "单选"
Operator *string `json:"operator,omitempty"` // 条件运算符示例值: "is"可选值有: 等于不等于包含不包含为空不为空默认值: `is`
Value []string `json:"value,omitempty"` // 条件的值, 可以是单个值或多个值的数组。详情参考[字段目标值（value）填写说明](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/app-table-record/record-filter-guide#3e0fd644)。示例值: ["optbdVHf4q"] 长度范围: `0` ～ `50`
}
...
// CreateBaseAppRoleResp ...
type CreateBaseAppRoleResp struct { 
Role *CreateBaseAppRoleRespRole `json:"role,omitempty"` // 自定义角色
}
...
// CreateBaseAppRoleRespRole ...
type CreateBaseAppRoleRespRole struct { 
RoleName string `json:"role_name,omitempty"` // 自定义角色名称
TableRoles []*CreateBaseAppRoleRespRoleTableRole `json:"table_roles,omitempty"` // 针对数据表的权限设置
RoleID string `json:"role_id,omitempty"` // 自定义权限的 ID
BlockRoles []*CreateBaseAppRoleRespRoleBlockRole `json:"block_roles,omitempty"` // 针对仪表盘的权限设置
...
// CreateBaseAppRoleRespRoleBlockRole ...
type CreateBaseAppRoleRespRoleBlockRole struct { 
BlockID string `json:"block_id,omitempty"` // 多维表格仪表盘的唯一标识
BlockPerm int64 `json:"block_perm,omitempty"` // 仪表盘的权限可选值有: 无权限可阅读
BlockType string `json:"block_type,omitempty"` // 仪表盘类型可选值有: 仪表盘
}
...
// CreateBaseAppRoleRespRoleTableRole ...
type CreateBaseAppRoleRespRoleTableRole struct { 
...
RecRule *CreateBaseAppRoleRespRoleTableRoleRecRule `json:"rec_rule,omitempty"` // 记录筛选条件, 当 `table_perm` 为 1 或 2 时生效。用于指定可编辑或可阅读的记录。
OtherRecRule *CreateBaseAppRoleRespRoleTableRoleOtherRecRule `json:"other_rec_rule,omitempty"` // 记录筛选条件, 在 `rec_rule.other_perm` 为 0 时生效。对于未命中 `rec_rule` 的记录, 通过 `other_rec_rule` 指定可阅读记录范围；此时, 既未命中 `rec_rule`、也未命中 `other_rec_rule` 的记录会被禁止阅读。
FieldPerm interface{} `json:"field_perm,omitempty"` // 字段权限, 仅在 `table_perm` 为 1 和 2 时生效。用于设置字段的可编辑或可阅读权限。类型为 map, key 是字段名称, value 是字段权限。value 枚举值有: `1`: 可阅读- `2`: 可添加- `3`: 可编辑
...
// CreateBaseAppRoleRespRoleTableRoleOtherRecRule ...
type CreateBaseAppRoleRespRoleTableRoleOtherRecRule struct { 
Conditions []*CreateBaseAppRoleRespRoleTableRoleOtherRecRuleCondition `json:"conditions,omitempty"` // 记录筛选条件, 用于指定可阅读的记录。
...
// CreateBaseAppRoleRespRoleTableRoleOtherRecRuleCondition ...
type CreateBaseAppRoleRespRoleTableRoleOtherRecRuleCondition struct { 
...
// CreateBaseAppRoleRespRoleTableRoleRecRule ...
type CreateBaseAppRoleRespRoleTableRoleRecRule struct { 
Conditions []*CreateBaseAppRoleRespRoleTableRoleRecRuleCondition `json:"conditions,omitempty"` // 记录筛选条件, 用于指定可编辑或可阅读的记录。
Conjunction string `json:"conjunction,omitempty"` // 多个筛选条件的关系可选值有: 与或
Perm int64 `json:"perm,omitempty"` // 命中 rec_rule 的记录对应的权限, 可不设置, 理论上应该与 table_perm 保持一致可选值有: 仅可阅读可编辑
...
type CreateBaseAppRoleRespRoleTableRoleRecRuleCondition struct { 
...
Data *CreateBaseAppRoleResp `json:"data,omitempty"` 
