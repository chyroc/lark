...
// GetDepartment 调用该接口获取单个部门信息, 包括部门名称、ID、父部门、负责人、状态以及成员个数等。
//
// ## 注意事项
...
type GetDepartmentRespDepartment struct {
	...
	I18nName               *GetDepartmentRespDepartmentI18nName `json:"i18n_name,omitempty"`                 // 部门名称的国际化配置。字段权限要求（满足任一）: 获取部门基础信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	...
	Status                 *GetDepartmentRespDepartmentStatus   `json:"status,omitempty"`                    // 部门状态。字段权限要求（满足任一）: 获取部门基础信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	Leaders                []*GetDepartmentRespDepartmentLeader `json:"leaders,omitempty"`                   // 部门负责人信息。
