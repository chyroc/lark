...
type UpdateUserPatchRespUser struct {
	...
	Avatar                  *UpdateUserPatchRespUserAvatar       `json:"avatar,omitempty"`                      // 用户头像信息, 字段权限要求（满足任一）: 获取用户基本信息, 以应用身份访问通讯录, 读取通讯录, 以应用身份读取通讯录
	Status                  *UpdateUserPatchRespUserStatus       `json:"status,omitempty"`                      // 用户状态, 枚举类型, 包括is_frozen、is_resigned、is_activated、is_exited, 用户状态转移参见: [用户状态图](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/field-overview#4302b5a1), 字段权限要求（满足任一）: 获取用户受雇信息, 以应用身份访问通讯录, 读取通讯录, 以应用身份读取通讯录
	...
	Orders                  []*UpdateUserPatchRespUserOrder      `json:"orders,omitempty"`                      // 用户排序信息, 用于标记通讯录下组织架构的人员顺序, 人员可能存在多个部门中, 且有不同的排序, 字段权限要求（满足任一）: 获取用户组织架构信息, 以应用身份访问通讯录, 读取通讯录, 以应用身份读取通讯录
	CustomAttrs             []*UpdateUserPatchRespUserCustomAttr `json:"custom_attrs,omitempty"`                // 自定义字段, 请确保你的组织管理员已在管理后台/组织架构/成员字段管理/自定义字段管理/全局设置中开启了“允许开放平台 API 调用“, 否则该字段不会生效/返回, 更多详情参见[用户接口相关问题](https://open.feishu.cn/document/ugTN1YjL4UTN24CO1UjN/uQzN1YjL0cTN24CN3UjN#77061525), 字段权限要求（满足任一）: 获取用户受雇信息, 以应用身份访问通讯录, 读取通讯录, 以应用身份读取通讯录
