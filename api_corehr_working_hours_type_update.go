...
type UpdateCoreHRWorkingHoursTypeReq struct {
	WorkingHoursTypeID  string                                        `path:"working_hours_type_id" json:"-"`   // 工时制度ID- 可通过[批量查询工时制度](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/working_hours_type/list)获取示例值: "1616161616"
	ClientToken         *string                                       `query:"client_token" json:"-"`           // 根据client_token是否一致来判断是否为同一请求示例值: 12454646
	Code                *string                                       `json:"code,omitempty"`                   // 编码示例值: "1"
	Name                []*UpdateCoreHRWorkingHoursTypeReqName        `json:"name,omitempty"`                   // 名称
	CountryRegionIDList []string                                      `json:"country_region_id_list,omitempty"` // 国家/地区 ID 列表- 可通过[查询国家/地区信息](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/corehr-v2/basic_info-country_region/search)获取示例值: ["6890452208593356295"]
	DefaultForJob       *bool                                         `json:"default_for_job,omitempty"`        // 职务默认值示例值: true
	Active              *bool                                         `json:"active,omitempty"`                 // 是否启用示例值: true
	CustomFields        []*UpdateCoreHRWorkingHoursTypeReqCustomField `json:"custom_fields,omitempty"`          // 自定义字段- 具体支持的对象请参考[自定义字段说明](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/custom-fields-guide)
}
...
type UpdateCoreHRWorkingHoursTypeRespWorkingHoursType struct {
	...
	CustomFields        []*UpdateCoreHRWorkingHoursTypeRespWorkingHoursTypeCustomField `json:"custom_fields,omitempty"`          // 自定义字段- 具体支持的对象请参考[自定义字段说明](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/custom-fields-guide)
}
