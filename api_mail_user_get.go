...
type GetMailUserReq struct {
	EmailList []string `json:"email_list,omitempty"` // 需要查询的邮箱地址列表示例值: ["test@a.com"]
}
...
type GetMailUserRespUser struct {
	...
	Status int64  `json:"status,omitempty"` // 邮箱地址状态可选值有: 邮箱地址格式错误邮箱地址域名不存在邮箱地址不存在启用已删除（邮箱回收站中）禁用
	Type   int64  `json:"type,omitempty"`   // 邮箱地址类型可选值有: 成员邮箱成员邮箱别名公共邮箱公共邮箱别名邮件组邮件组别名
}
