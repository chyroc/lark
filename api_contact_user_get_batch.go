...
type BatchGetUserRespItem struct {
	...
	Avatar          *BatchGetUserRespItemAvatar           `json:"avatar,omitempty"`            // 用户头像信息, 字段权限要求（满足任一）: 获取用户基本信息, 以应用身份访问通讯录, 读取通讯录, 以应用身份读取通讯录
	Status          *BatchGetUserRespItemStatus           `json:"status,omitempty"`            // 用户状态, 字段权限要求（满足任一）: 获取用户受雇信息, 以应用身份访问通讯录, 读取通讯录, 以应用身份读取通讯录
	...
	City            string                                `json:"city,omitempty"`              // 城市, 字段权限要求（满足任一）: 获取用户受雇信息, 以应用身份访问通讯录, 读取通讯录, 以应用身份读取通讯录
	...
	Orders          []*BatchGetUserRespItemOrder          `json:"orders,omitempty"`            // 用户排序信息, 用于标记通讯录下组织架构的人员顺序, 人员可能存在多个部门中, 且有不同的排序, 字段权限要求（满足任一）: 获取用户组织架构信息, 以应用身份访问通讯录, 读取通讯录, 以应用身份读取通讯录
	CustomAttrs     []*BatchGetUserRespItemCustomAttr     `json:"custom_attrs,omitempty"`      // 自定义属性, 字段权限要求（满足任一）: 获取用户受雇信息, 以应用身份访问通讯录, 读取通讯录, 以应用身份读取通讯录
