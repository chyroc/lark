...
type ExportVCMeetingListReq struct {
	...
	MeetingStatus *int64  `json:"meeting_status,omitempty"` // 会议状态（不传默认为已结束会议）示例值: 2可选值有: 进行中已结束待召开
	...
	MeetingType   *int64  `json:"meeting_type,omitempty"`   // 按会议类型筛选（最多一个筛选条件）示例值: 2可选值有: 全部类型（默认）视频会议本地投屏
}
