...
type ExportVCMeetingListReq struct {
	...
	MeetingStatus *int64  `json:"meeting_status,omitempty"` // 会议状态（不传默认为已结束会议）, 示例值: 2, 可选值有: 1: 进行中, 2: 已结束, 3: 待召开
	...
	MeetingType   *int64  `json:"meeting_type,omitempty"`   // 按会议类型筛选（最多一个筛选条件）, 示例值: 2, 可选值有: 1: 全部类型（默认）, 2: 视频会议, 3: 本地投屏
}
