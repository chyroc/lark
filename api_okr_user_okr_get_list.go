...
type GetUserOKRListReq struct {
	UserID     string   `path:"user_id" json:"-"`       // 目标用户id示例值: "ou-asdasdasdasdasd"
	...
	Offset     string   `query:"offset" json:"-"`       // 请求列表的偏移（对应响应体的 okr_list 字段）, offset>=0示例值: 0
	Limit      string   `query:"limit" json:"-"`        // 列表长度, 0-10示例值: 5
	Lang       *string  `query:"lang" json:"-"`         // 请求OKR的语言版本（比如@的人名）, lang=en_us/zh_cn示例值: zh_cn默认值: `zh_cn`
	PeriodIDs  []string `query:"period_ids" json:"-"`   // period_id列表, 最多10个示例值: 6990181559036610092 最大长度: `10`
}
