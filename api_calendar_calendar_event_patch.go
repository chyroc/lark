...
// UpdateCalendarEvent 以当前身份（应用或用户）更新指定日历上的一个日程, 包括日程标题、描述、开始与结束时间、视频会议以及日程地点等信息。
//
// ## 前提条件
...
type UpdateCalendarEventReq struct {
	CalendarID       string                              `path:"calendar_id" json:"-"`        // 日程所在的日历 ID。ID 获取方式: [创建共享日历](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar/create)时, 接口会返回日历 ID。- 调用[查询主日历信息](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar/primary)、[查询日历列表](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar/list)或者[搜索日历](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar/search)接口, 获取所需的日历 ID。- 示例值: "feishu.cn_xxxxxxxxxx@group.calendar.feishu.cn"
	EventID          string                              `path:"event_id" json:"-"`           // 日程 ID。ID 获取方式: [创建日程](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar-event/create)时, 接口会返回日程 ID。- 调用[获取日程列表](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar-event/list)或者[搜索日程](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar-event/search)接口, 获取所需的日程 ID。示例值: "00592a0e-7edf-4678-bc9d-1b77383ef08e_0"
	...
	Summary          *string                             `json:"summary,omitempty"`           // 日程标题。默认值: 空, 表示不更新该字段示例值: "团队周会" 最大长度: `1000` 字符
	Description      *string                             `json:"description,omitempty"`       // 日程描述。注意: 目前 API 方式不支持编辑富文本描述。如果日程描述通过客户端编辑为富文本内容, 则使用 API 更新描述会导致富文本格式丢失。默认值: 空, 表示不更新该字段示例值: "讨论项目进展" 最大长度: `40960` 字符
	NeedNotification *bool                               `json:"need_notification,omitempty"` // 更新日程时, 是否给日程参与人发送 Bot 通知。默认值: 空, 表示不更新该字段可选值有: true: 发送通知- false: 不发送通知示例值: false
	...
	Visibility       *string                             `json:"visibility,omitempty"`        // 日程公开范围。注意: 更新日程时如果修改了该参数值, 则仅对当前身份生效。默认值: 空, 表示不更新该字段示例值: "default"可选值有: 默认权限, 即跟随日历权限, 默认仅向他人显示是否忙碌公开, 显示日程详情私密, 仅自己可见详情
	AttendeeAbility  *string                             `json:"attendee_ability,omitempty"`  // 参与人权限。默认值: 空, 表示不更新该字段示例值: "can_see_others"可选值有: 无法编辑日程、无法邀请其他参与人、无法查看参与人列表无法编辑日程、无法邀请其他参与人、可以查看参与人列表无法编辑日程、可以邀请其他参与人、可以查看参与人列表可以编辑日程、可以邀请其他参与人、可以查看参与人列表
	FreeBusyStatus   *string                             `json:"free_busy_status,omitempty"`  // 日程占用的忙闲状态, 新建日程默认为 `busy`。注意: 更新日程时如果修改了该参数值, 则仅对当前身份生效。默认值: 空, 表示不更新该字段示例值: "busy"可选值有: 忙碌空闲
	Location         *UpdateCalendarEventReqLocation     `json:"location,omitempty"`          // 日程地点。不传值则表示不更新该字段。
	Color            *int64                              `json:"color,omitempty"`             // 日程颜色, 取值通过颜色 RGB 值的 int32 表示。注意: 该参数仅对当前身份生效。- 客户端展示时会映射到色板上最接近的一种颜色。- 取值为 0 或 -1 时, 默认跟随日历颜色。默认值: 空, 表示不更新该字段示例值:1
	Reminders        []*UpdateCalendarEventReqReminder   `json:"reminders,omitempty"`         // 日程提醒列表。不传值则表示不更新该字段。
	Recurrence       *string                             `json:"recurrence,omitempty"`        // 重复日程的重复性规则, 规则设置方式参考[rfc5545](https://datatracker.ietf.org/doc/html/rfc5545#section-3.3.10)。注意: COUNT 和 UNTIL 不支持同时出现。- 预定会议室重复日程长度不得超过两年。默认值: 空, 表示不更新该字段示例值: "FREQ=DAILY;INTERVAL=1" 最大长度: `2000` 字符
	Schemas          []*UpdateCalendarEventReqSchema     `json:"schemas,omitempty"`           // 日程自定义信息, 控制日程详情页的 UI 展示。schemas字段不传值则表示不更新该字段。注意: 1. schemas传值的情况下, 每次都是覆盖更新, 即用传入的列表去更新原来的列表。2. 可以使用[]空列表来清空schemas原来的数据。
	Attachments      []*UpdateCalendarEventReqAttachment `json:"attachments,omitempty"`       // 日程附件。
	EventCheckIn     *UpdateCalendarEventReqEventCheckIn `json:"event_check_in,omitempty"`    // 日程签到设置, 为空则不进行日程签到设置。
}

// UpdateCalendarEventReqAttachment ...
type UpdateCalendarEventReqAttachment struct {
	FileToken *string `json:"file_token,omitempty"` // 附件 Token。调用[上传素材](https://open.larkoffice.com/document/server-docs/docs/drive-v1/media/upload_all)接口, 获取附件的 file_token。在调用上传素材接口时需要注意: `parent_type` 需传入固定值 `calender`。- `parent_node` 需传入与当前接口一致的日历 ID。附件校验规则: 附件总大小不超过 25 MB。示例值: "boxcnrHpsg1QDqXAAAyachaxxxx"
	IsDeleted *bool   `json:"is_deleted,omitempty"` // 是否删除附件。可选值有: true: 删除- false: 不删除默认值: false示例值: true
}
...
// UpdateCalendarEventReqEventCheckIn ...
type UpdateCalendarEventReqEventCheckIn struct {
	EnableCheckIn       bool                                                `json:"enable_check_in,omitempty"`       // 是否启用日程签到。示例值: true
	CheckInStartTime    *UpdateCalendarEventReqEventCheckInCheckInStartTime `json:"check_in_start_time,omitempty"`   // 日程签到开始时间。注意: 签到开始时间不能大于或者等于签到结束时间。
	CheckInEndTime      *UpdateCalendarEventReqEventCheckInCheckInEndTime   `json:"check_in_end_time,omitempty"`     // 日程签到结束时间。注意: 签到开始时间不能大于或者等于签到结束时间。
	NeedNotifyAttendees *bool                                               `json:"need_notify_attendees,omitempty"` // 签到开始时是否自动发送签到通知给参与者示例值: false默认值: `false`
}

// UpdateCalendarEventReqEventCheckInCheckInEndTime ...
type UpdateCalendarEventReqEventCheckInCheckInEndTime struct {
	TimeType string `json:"time_type,omitempty"` // 偏移量(分钟)相对于的日程时间节点类型。示例值: "after_event_end"可选值有: 日程开始前日程开始后日程结束后
	Duration int64  `json:"duration,omitempty"`  // 相对于日程开始或者结束的偏移量(分钟)。- 目前取值只能为列表[0, 5, 15, 30, 60]之一, 0表示立即开始。- 当time_type为before_event_start, duration不能取0。示例值: 0 取值范围: `0` ～ `60`
}

// UpdateCalendarEventReqEventCheckInCheckInStartTime ...
type UpdateCalendarEventReqEventCheckInCheckInStartTime struct {
	TimeType string `json:"time_type,omitempty"` // 偏移量(分钟)相对于的日程时间节点类型。示例值: "before_event_start"可选值有: 日程开始前日程开始后日程结束后
	Duration int64  `json:"duration,omitempty"`  // 相对于日程开始或者结束的偏移量(分钟)。- 目前取值只能为列表[0, 5, 15, 30, 60]之一, 0表示立即开始。- 当time_type为before_event_start, duration不能取0。示例值: 15 取值范围: `0` ～ `60`
}

// UpdateCalendarEventReqLocation ...
type UpdateCalendarEventReqLocation struct {
	Name      *string  `json:"name,omitempty"`      // 地点名称。示例值: "301会议室" 长度范围: `1` ～ `512` 字符
	Address   *string  `json:"address,omitempty"`   // 地点地址。示例值: "公司大楼三层" 长度范围: `1` ～ `255` 字符
...
type UpdateCalendarEventReqVchat struct {
	VCType          *string                                     `json:"vc_type,omitempty"`          // 视频会议类型。如果无需视频会议, 则必须传入 `no_meeting`。示例值: "third_party"可选值有: 飞书视频会议。取该类型时, vchat 内的其他字段均无效。第三方链接视频会议。取该类型时, 仅生效 vchat 内的 icon_type、description、meeting_url 字段。无视频会议。取该类型时, vchat 内的其他字段均无效。飞书直播。该值用于客户端, 不支持通过 API 调用, 只读。未知类型。该值用于客户端做兼容使用, 不支持通过 API 调用, 只读。
	IconType        *string                                     `json:"icon_type,omitempty"`        // 第三方视频会议的 icon 类型。默认值: 空, 表示不更新该字段示例值: "vc"可选值有: 飞书视频会议 icon直播视频会议 icon默认 icon
	Description     *string                                     `json:"description,omitempty"`      // 第三方视频会议文案。默认值: 空, 表示不更新该字段示例值: "发起视频会议" 长度范围: `0` ～ `500` 字符
	MeetingURL      *string                                     `json:"meeting_url,omitempty"`      // 视频会议 URL。默认值: 空, 表示不更新该字段示例值: "https://example.com" 长度范围: `1` ～ `2000` 字符
...
type UpdateCalendarEventReqVchatMeetingSettings struct {
	...
	JoinMeetingPermission *string  `json:"join_meeting_permission,omitempty"` // 设置入会范围。示例值: "only_organization_employees"可选值有: 所有人可以加入会议仅企业内用户可以加入会议仅日程参与者可以加入会议
	...
	AutoRecord            *bool    `json:"auto_record,omitempty"`             // 是否开启自动录制。可选值有: true: 开启- false: 不开启默认值: 空, 表示不更新该字段示例值: false
	OpenLobby             *bool    `json:"open_lobby,omitempty"`              // 是否开启等候室。可选值有: true: 开启- false: 不开启默认值: 空, 表示不更新该字段示例值: true
	AllowAttendeesStart   *bool    `json:"allow_attendees_start,omitempty"`   // 是否允许日程参与者发起会议。注意: 应用日历上操作日程时, 该字段必须为 true, 否则没有人能发起会议。可选值有: true: 允许- false: 不允许默认值: 空, 表示不更新该字段示例值: true
}
...
type UpdateCalendarEventRespEvent struct {
	...
	Visibility          string                                      `json:"visibility,omitempty"`            // 日程公开范围, 仅对当前身份生效。可选值有: 默认权限, 跟随日历权限, 即默认仅向他人显示是否忙碌公开, 显示日程详情私密, 仅自己可见详情
	...
	FreeBusyStatus      string                                      `json:"free_busy_status,omitempty"`      // 日程占用的忙闲状态, 仅对当前身份生效。可选值有: 忙碌空闲
	...
	EventCheckIn        *UpdateCalendarEventRespEventEventCheckIn   `json:"event_check_in,omitempty"`        // 日程签到设置。
}
...
// UpdateCalendarEventRespEventEventCheckIn ...
type UpdateCalendarEventRespEventEventCheckIn struct {
	EnableCheckIn       bool                                                      `json:"enable_check_in,omitempty"`       // 是否启用日程签到。
	CheckInStartTime    *UpdateCalendarEventRespEventEventCheckInCheckInStartTime `json:"check_in_start_time,omitempty"`   // 日程签到开始时间。
	CheckInEndTime      *UpdateCalendarEventRespEventEventCheckInCheckInEndTime   `json:"check_in_end_time,omitempty"`     // 日程签到结束时间。
	NeedNotifyAttendees bool                                                      `json:"need_notify_attendees,omitempty"` // 签到开始时是否自动发送签到通知给参与者
}

// UpdateCalendarEventRespEventEventCheckInCheckInEndTime ...
type UpdateCalendarEventRespEventEventCheckInCheckInEndTime struct {
...
// UpdateCalendarEventRespEventEventCheckInCheckInStartTime ...
type UpdateCalendarEventRespEventEventCheckInCheckInStartTime struct {
	TimeType string `json:"time_type,omitempty"` // 偏移量(分钟)相对于的日程时间节点类型。可选值有: 日程开始前日程开始后日程结束后
	Duration int64  `json:"duration,omitempty"`  // 相对于日程开始或者结束的偏移量(分钟)。
}
...
type UpdateCalendarEventRespEventVchatMeetingSettings struct {
	...
	JoinMeetingPermission string   `json:"join_meeting_permission,omitempty"` // 设置入会范围。可选值有: 所有人可以加入会议仅企业内用户可以加入会议仅日程参与者可以加入会议
