...
// BatchSaveCoreHRReportDetailRow 批量创建/更新填报行后, 可在【设置-编制规划设置-编制规划XXX-集中填报-查看数据】进行查看。
//
...
func (r *CoreHRService) BatchSaveCoreHRReportDetailRow(ctx context.Context, request *BatchSaveCoreHRReportDetailRowReq, options ...MethodOptionFunc) (*BatchSaveCoreHRReportDetailRowResp, *Response, error) {
	if r.cli.mock.mockCoreHRBatchSaveCoreHRReportDetailRow != nil {
		r.cli.Log(ctx, LogLevelDebug, "[lark] CoreHR#BatchSaveCoreHRReportDetailRow mock enable")
		return r.cli.mock.mockCoreHRBatchSaveCoreHRReportDetailRow(ctx, request, options...)
	}

	req := &RawRequestReq{
		Scope:                 "CoreHR",
		API:                   "BatchSaveCoreHRReportDetailRow",
		Method:                "POST",
	...
	resp := new(batchSaveCoreHRReportDetailRowResp)

	response, err := r.cli.RawRequest(ctx, req, resp)
	return resp.Data, response, err
}

// MockCoreHRBatchSaveCoreHRReportDetailRow mock CoreHRBatchSaveCoreHRReportDetailRow method
func (r *Mock) MockCoreHRBatchSaveCoreHRReportDetailRow(f func(ctx context.Context, request *BatchSaveCoreHRReportDetailRowReq, options ...MethodOptionFunc) (*BatchSaveCoreHRReportDetailRowResp, *Response, error)) {
	r.mockCoreHRBatchSaveCoreHRReportDetailRow = f
}

// UnMockCoreHRBatchSaveCoreHRReportDetailRow un-mock CoreHRBatchSaveCoreHRReportDetailRow method
func (r *Mock) UnMockCoreHRBatchSaveCoreHRReportDetailRow() {
	r.mockCoreHRBatchSaveCoreHRReportDetailRow = nil
}

// BatchSaveCoreHRReportDetailRowReq ...
type BatchSaveCoreHRReportDetailRowReq struct {
	...
	Items                         []*BatchSaveCoreHRReportDetailRowReqItem `json:"items,omitempty"`                            // 集中填报的填报行数量应介于 1 至 5 个之间。 长度范围: `1` ～ `5`
}

// BatchSaveCoreHRReportDetailRowReqItem ...
type BatchSaveCoreHRReportDetailRowReqItem struct {
	Dimensions        []*BatchSaveCoreHRReportDetailRowReqItemDimension        `json:"dimensions,omitempty"`          // 集中填报的维度信息要和用户创建的维度匹配, 即传入除自动匹配维度外的所有维度, 不多不少。通过「设置-编制规划设置-编制规划XXX」查看该编制规划有哪些维度。自定义组织暂时不支持【为组织设置自动匹配规则】, 请联系[技术支持](https://applink.feishu.cn/TLJpeNdW)了解如何判断该字段是否为自动匹配字段。 长度范围: `1` ～ `20`
	EaiDetails        []*BatchSaveCoreHRReportDetailRowReqItemEaiDetail        `json:"eai_details,omitempty"`         // 预估在职人数相关信息。可从「设置-编制规划设置-编制规划XXX-集中填报」查看预估在职人数的时间, 如果不存在该字段说明用户创建时即没有允许填写该字段, 批量创建/更新填报行时则无需给该字段, 如果存在, 用户需要查看预估在职人数的日期, 使用示例值格式进行传参。 长度范围: `0` ～ `15`
	...
	MultiPeriodValues []*BatchSaveCoreHRReportDetailRowReqItemMultiPeriodValue `json:"multi_period_values,omitempty"` // 自然周期的编制规划信息。当编制规划方案是按自然周期选择时, 设置该字段。 长度范围: `0` ～ `15`
}

// BatchSaveCoreHRReportDetailRowReqItemDimension ...
type BatchSaveCoreHRReportDetailRowReqItemDimension struct {
...
type BatchSaveCoreHRReportDetailRowReqItemEaiDetail struct {
...
// BatchSaveCoreHRReportDetailRowReqItemMultiPeriodValue ...
type BatchSaveCoreHRReportDetailRowReqItemMultiPeriodValue struct {
...
// BatchSaveCoreHRReportDetailRowResp ...
type BatchSaveCoreHRReportDetailRowResp struct {
}
...
type batchSaveCoreHRReportDetailRowResp struct {
	...
	Data  *BatchSaveCoreHRReportDetailRowResp `json:"data,omitempty"`
