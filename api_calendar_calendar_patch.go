...
type UpdateCalendarReq struct {
	...
	Summary      *string             `json:"summary,omitempty"`       // 日历标题。- 不传表示不更新该字段。示例值: "测试日历" 长度范围: `1` ～ `255` 字符
	Description  *string             `json:"description,omitempty"`   // 日历描述。- 不传表示不更新该字段。示例值: "使用开放接口创建日历" 最大长度: `255` 字符
	Permissions  *CalendarPermission `json:"permissions,omitempty"`   // 日历公开范围。- 不传表示不更新该字段。示例值: "private"可选值有: 私密仅展示忙闲信息公开, 他人可查看日程详情
	Color        *int64              `json:"color,omitempty"`         // 日历颜色, 取值通过颜色 RGB 值的 int32 表示, 其中, 24 ~ 31 位为透明度, 16 ~ 23 位为红, 8 ~ 15 位为绿, 0 ~ 7 位为蓝。例如, 11034625 表示 RGB 值 (87, 159, 255)。- 不传表示不更新该字段。注意: 取值范围为 -2^31 ~ 2^31-1- 日历颜色会映射到飞书客户端色板上最接近的一种颜色进行展示。- 该颜色仅对当前身份生效。示例值:1
	SummaryAlias *string             `json:"summary_alias,omitempty"` // 日历备注名, 设置该字段后（包括后续修改该字段）仅对当前身份生效。- 不传表示不更新该字段。示例值: "日历备注名" 最大长度: `255` 字符
}
