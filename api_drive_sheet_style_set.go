...
func (r *DriveService) SetSheetStyle(ctx context.Context, request *SetSheetStyleReq, options ...MethodOptionFunc) (*SetSheetStyleResp, *Response, error) {
	...
	r.cli.log(ctx, LogLevelInfo, "[lark] Drive#SetSheetStyle call api")
	r.cli.log(ctx, LogLevelDebug, "[lark] Drive#SetSheetStyle request: %s", jsonString(request))
	...
	if err != nil {
		r.cli.log(ctx, LogLevelError, "[lark] Drive#SetSheetStyle PUT https://open.feishu.cn/open-apis/sheets/v2/spreadsheets/:spreadsheetToken/style failed, request_id: %s, status_code: %d, error: %s", requestID, statusCode, err)
		return nil, response, err
	} else if resp.Code != 0 {
		r.cli.log(ctx, LogLevelError, "[lark] Drive#SetSheetStyle PUT https://open.feishu.cn/open-apis/sheets/v2/spreadsheets/:spreadsheetToken/style failed, request_id: %s, status_code: %d, code: %d, msg: %s", requestID, statusCode, resp.Code, resp.Msg)
		return nil, response, NewError("Drive", "SetSheetStyle", resp.Code, resp.Msg)
	}

	r.cli.log(ctx, LogLevelDebug, "[lark] Drive#SetSheetStyle success, request_id: %s, status_code: %d, response: %s", requestID, statusCode, jsonString(resp.Data))
