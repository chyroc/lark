...
type GetTaskReminderListReq struct {
	TaskID    string  `path:"task_id" json:"-"`     // 任务 ID示例值: "0d38e26e-190a-49e9-93a2-35067763ed1f"
	PageSize  *int64  `query:"page_size" json:"-"`  // 分页大小示例值: 50默认值: `50` 取值范围: `0` ～ `50`
	PageToken *string `query:"page_token" json:"-"` // 分页标记, 第一次请求不填, 表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token, 下次遍历可采用该 page_token 获取查询结果示例值: 「填写上次返回的page_token」
}
