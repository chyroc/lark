...
// UpdateBitableAppRole 更新多维表格高级权限中自定义的角色。
//
// 更新自定义角色为增量更新, 仅对传值的字段进行更新, 不传值则不更新。推荐使用新版[更新自定义角色](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/advanced-permission/base-v2/app-role/update)接口, 支持高级权限 2.0 版本新增的权限点位, 包括更精细的行级别权限控制、多维表格的复制、导出点位的控制等。
// ## 前提条件
...
type UpdateBitableAppRoleReq struct {
	...
	RoleName   string                              `json:"role_name,omitempty"`   // 自定义角色名称示例值: "自定义角色1"
	TableRoles []*UpdateBitableAppRoleReqTableRole `json:"table_roles,omitempty"` // 针对数据表的权限设置 最大长度: `100`
	BlockRoles []*UpdateBitableAppRoleReqBlockRole `json:"block_roles,omitempty"` // 针对仪表盘的权限设置 最大长度: `100`
}
...
type UpdateBitableAppRoleReqTableRole struct {
	TablePerm         int64                                    `json:"table_perm,omitempty"`          // 数据表权限。提示: 协作者可编辑自己的记录 和 可编辑指定字段 是 可编辑记录 的特殊情况, 可通过指定 `rec_rule` 或 `field_perm` 参数实现相同的效果。示例值: 0可选值有: 无权限可阅读可编辑记录可编辑字段和记录默认值: `0`
	...
	RecRule           *UpdateBitableAppRoleReqTableRoleRecRule `json:"rec_rule,omitempty"`            // 记录筛选条件, 当 `table_perm` 为 1 或 2 时生效。用于指定可编辑或可阅读的记录。
...
type UpdateBitableAppRoleReqTableRoleRecRule struct {
	Conditions  []*UpdateBitableAppRoleReqTableRoleRecRuleCondition `json:"conditions,omitempty"`  // 记录筛选条件, 用于指定可编辑或可阅读的记录。 最大长度: `100`
...
type UpdateBitableAppRoleResp struct {
	Role *UpdateBitableAppRoleRespRole `json:"role,omitempty"` // 自定义角色列表
}
...
type UpdateBitableAppRoleRespRole struct {
	...
	TableRoles []*UpdateBitableAppRoleRespRoleTableRole `json:"table_roles,omitempty"` // 针对数据表的权限设置
	BlockRoles []*UpdateBitableAppRoleRespRoleBlockRole `json:"block_roles,omitempty"` // 针对仪表盘的权限设置
}
...
type UpdateBitableAppRoleRespRoleTableRole struct {
	...
	RecRule           *UpdateBitableAppRoleRespRoleTableRoleRecRule `json:"rec_rule,omitempty"`            // 可编辑或可阅读的记录。当 `table_perm` 为 1 或 2 时生效。
...
type UpdateBitableAppRoleRespRoleTableRoleRecRule struct {
	Conditions  []*UpdateBitableAppRoleRespRoleTableRoleRecRuleCondition `json:"conditions,omitempty"`  // 记录筛选条件, 用于指定可编辑或可阅读的记录。
