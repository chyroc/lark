...
// GetPerformanceStageTaskByPage 批量获取周期下所有用户的任务信息。支持传入任务分类、任务截止时间参数删选周期内任务数据。
//
// doc: https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/performance-v1/stage_task/find_by_page
// new doc: https://open.feishu.cn/document/performance-v1/stage_task/find_by_page
...
type GetPerformanceStageTaskByPageReq struct {
	...
	PageToken       *string `json:"page_token,omitempty"`        // 分页标记, 第一次请求不填, 表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token, 下次遍历可采用该 page_token 获取查询结果示例值: "AQD9/Rn9eij9Pm39ED40/dk53s4Ebp882DYfFaPFbz00L4CMZJrqGdzNyc8BcZtDbwVUvRmQTvyMYicnGWrde9X56TgdBuS+JKiSIkdexPw="
	PageSize        *int64  `json:"page_size,omitempty"`         // 分页大小示例值: 30默认值: `20` 最大值: `50`
}
