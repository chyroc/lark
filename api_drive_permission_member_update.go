...
func (r *DriveService) UpdateMemberPermission(ctx context.Context, request *UpdateMemberPermissionReq, options ...MethodOptionFunc) (*UpdateMemberPermissionResp, *Response, error) {
	...
	r.cli.logInfo(ctx, "[lark] Drive#UpdateMemberPermission call api")
	r.cli.logDebug(ctx, "[lark] Drive#UpdateMemberPermission request: %s", jsonString(request))
	...
	if err != nil {
		r.cli.logError(ctx, "[lark] Drive#UpdateMemberPermission POST https://open.feishu.cn/open-apis/drive/permission/member/update failed: %s", err)
		return nil, response, err
	} else if resp.Code != 0 {
		r.cli.logError(ctx, "[lark] Drive#UpdateMemberPermission POST https://open.feishu.cn/open-apis/drive/permission/member/update failed, code: %d, msg: %s", resp.Code, resp.Msg)
		return nil, response, NewError("Drive", "UpdateMemberPermission", resp.Code, resp.Msg)
	}

	r.cli.logDebug(ctx, "[lark] Drive#UpdateMemberPermission request_id: %s, response: %s", response.RequestID, jsonString(resp.Data))
