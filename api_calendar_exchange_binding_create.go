...
// CreateCalendarExchangeBinding 调用该接口将 Exchange 账户绑定到飞书账户, 进而支持 Exchange 日历的导入。
//
// 当前身份需要是企业超级管理员。
//
...
type CreateCalendarExchangeBindingReq struct {
	...
	AdminAccount    *string `json:"admin_account,omitempty"`    // Exchange 的 admin 账户, 示例值: "email_admin_example@outlook.com", 长度范围: `1` ～ `500` 字符
	ExchangeAccount *string `json:"exchange_account,omitempty"` // 需绑定的 Exchange 账户, 示例值: "email_account_example@outlook.com", 长度范围: `1` ～ `500` 字符
	UserID          *string `json:"user_id,omitempty"`          // 用户 ID, 即 Exchange 账户绑定的飞书账户 ID。关于用户 ID 可参见[用户相关的 ID 概念](https://open.feishu.cn/document/home/user-identity-introduction/introduction), 示例值: "ou_xxxxxxxxxxxxxxxxxx"
}
...
type CreateCalendarExchangeBindingResp struct {
	...
	UserID            string `json:"user_id,omitempty"`             // 用户 ID, 即 Exchange 账户绑定的飞书账户 ID。
	...
	ExchangeBindingID string `json:"exchange_binding_id,omitempty"` // Exchange 绑定的唯一标识 ID, 是 admin 账户、Exchange 账户、用户三元组的唯一标识 ID。你可以通过该 ID 查询绑定关系、日历同步状态, 或者解除绑定关系。
}
