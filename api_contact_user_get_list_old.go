...
type GetUserListOldReq struct {
	UserIDType       *IDType           `query:"user_id_type" json:"-"`       // 用户 ID 类型示例值: "open_id"可选值有: `open_id`: 用户的 open id- `union_id`: 用户的 union id- `user_id`: 用户的 user id默认值: `open_id`当值为 `user_id`, 字段权限要求: 获取用户 user ID
	DepartmentIDType *DepartmentIDType `query:"department_id_type" json:"-"` // 此次调用中使用的部门ID的类型示例值: "open_department_id"可选值有: `department_id`: 以自定义department_id来标识部门- `open_department_id`: 以open_department_id来标识部门默认值: `open_department_id
	DepartmentID     *string           `query:"department_id" json:"-"`      // 填写该字段表示获取部门下所有用户, 选填。示例值: "od-xxxxxxxxxxxxx"
	PageToken        *string           `query:"page_token" json:"-"`         // 分页标记, 第一次请求不填, 表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token, 下次遍历可采用该 page_token 获取查询结果示例值: "AQD9/Rn9eij9Pm39ED40/dk53s4Ebp882DYfFaPFbz00L4CMZJrqGdzNyc8BcZtDbwVUvRmQTvyMYicnGWrde9X56TgdBuS%2BJKiSIkdexPw="
...
type GetUserListOldRespItem struct {
	...
	Name            string                              `json:"name,omitempty"`              // 用户名。字段权限要求（满足任一）: 获取用户基本信息以应用身份读取通讯录读取通讯录以应用身份访问通讯录
	EnName          string                              `json:"en_name,omitempty"`           // 英文名。字段权限要求（满足任一）: 获取用户基本信息以应用身份读取通讯录读取通讯录以应用身份访问通讯录
	...
	Gender          int64                               `json:"gender,omitempty"`            // 性别。可选值有: `0`: 保密。- `1`: 男。- `2`: 女。字段权限要求（满足任一）: 获取用户性别以应用身份读取通讯录读取通讯录以应用身份访问通讯录
	Avatar          *GetUserListOldRespItemAvatar       `json:"avatar,omitempty"`            // 用户头像信息。字段权限要求（满足任一）: 获取用户基本信息以应用身份读取通讯录读取通讯录以应用身份访问通讯录
	Status          *GetUserListOldRespItemStatus       `json:"status,omitempty"`            // 用户状态。通过 is_frozen、is_resigned、is_activated、is_exited 布尔值类型参数进行展示。用户状态的转关逻辑可参见[用户资源介绍](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/field-overview#4302b5a1)。字段权限要求（满足任一）: 获取用户雇佣信息以应用身份读取通讯录读取通讯录以应用身份访问通讯录
	DepartmentIDs   []string                            `json:"department_ids,omitempty"`    // 用户所属部门的ID列表, 一个用户可属于多个部门。ID值的类型与查询参数中的department_id_type 对应。不同 ID 的说明与department_id的获取方式参见 [部门ID说明](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/department/field-overview#23857fe0)字段权限要求（满足任一）: 获取用户组织架构信息以应用身份读取通讯录读取通讯录以应用身份访问通讯录
	LeaderUserID    string                              `json:"leader_user_id,omitempty"`    // 用户的直接主管的 open_id。关于用户 ID 的说明可参见 [用户相关的 ID 概念](https://open.feishu.cn/document/home/user-identity-introduction/introduction)。字段权限要求（满足任一）: 获取用户组织架构信息以应用身份读取通讯录读取通讯录以应用身份访问通讯录
	City            string                              `json:"city,omitempty"`              // 工作城市。字段权限要求（满足任一）: 获取用户雇佣信息以应用身份读取通讯录读取通讯录以应用身份访问通讯录
	Country         string                              `json:"country,omitempty"`           // 国家或地区 Code 缩写。具体格式可参见 [国家/地区码表](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/country-code-description)。字段权限要求（满足任一）: 获取用户雇佣信息以应用身份读取通讯录读取通讯录以应用身份访问通讯录
	WorkStation     string                              `json:"work_station,omitempty"`      // 工位。字段权限要求（满足任一）: 获取用户雇佣信息以应用身份读取通讯录读取通讯录以应用身份访问通讯录
	JoinTime        int64                               `json:"join_time,omitempty"`         // 入职时间。秒级时间戳格式, 表示从 1970 年 1 月 1 日开始所经过的秒数。字段权限要求（满足任一）: 获取用户雇佣信息以应用身份读取通讯录读取通讯录以应用身份访问通讯录
	IsTenantManager bool                                `json:"is_tenant_manager,omitempty"` // 是否是租户超级管理员字段权限要求（满足任一）: 获取用户雇佣信息以应用身份读取通讯录读取通讯录以应用身份访问通讯录
	EmployeeNo      string                              `json:"employee_no,omitempty"`       // 工号。字段权限要求（满足任一）: 获取用户雇佣信息以应用身份读取通讯录读取通讯录以应用身份访问通讯录
	EmployeeType    int64                               `json:"employee_type,omitempty"`     // 员工类型。可能值有: `1`: 正式员工- `2`: 实习生- `3`: 外包- `4`: 劳务- `5`: 顾问   同时可读取到自定义员工类型的 int 值, 通过 int 值调用[获取人员类型](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/employee_type_enum/list)接口, 进而获取到该租户的自定义员工类型的名称。字段权限要求（满足任一）: 获取用户雇佣信息以应用身份读取通讯录读取通讯录以应用身份访问通讯录
	Orders          []*GetUserListOldRespItemOrder      `json:"orders,omitempty"`            // 用户排序信息。用于标记通讯录下组织架构的人员顺序, 人员可能存在多个部门中, 且有不同的排序。字段权限要求（满足任一）: 获取用户组织架构信息以应用身份读取通讯录读取通讯录以应用身份访问通讯录
	CustomAttrs     []*GetUserListOldRespItemCustomAttr `json:"custom_attrs,omitempty"`      // 自定义字段。如果企业管理员已在管理后台 > 组织架构 > 成员字段管理 > 自定义字段管理 > 全局设置中开启了 允许开放平台 API 调用, 则该字段生效。更多信息可参见[用户接口相关问题](https://open.feishu.cn/document/ugTN1YjL4UTN24CO1UjN/uQzN1YjL0cTN24CN3UjN#77061525)。字段权限要求（满足任一）: 获取用户雇佣信息以应用身份读取通讯录读取通讯录以应用身份访问通讯录
	EnterpriseEmail string                              `json:"enterprise_email,omitempty"`  // 企业邮箱。如果企业管理员已在管理后台启用飞书邮箱服务, 则会返回该值。字段权限要求（满足任一）: 获取用户雇佣信息以应用身份读取通讯录读取通讯录以应用身份访问通讯录
	JobTitle        string                              `json:"job_title,omitempty"`         // 职务。字段权限要求（满足任一）: 获取用户雇佣信息以应用身份读取通讯录读取通讯录以应用身份访问通讯录
}
...
type GetUserListOldRespItemCustomAttrValue struct {
	...
	GenericUser *GetUserListOldRespItemCustomAttrValueGenericUser `json:"generic_user,omitempty"` // 字段类型为 `GENERIC_USER` 时, 该参数返回定义的引用人员信息。
}
