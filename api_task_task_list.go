...
// ListTask 以分页的方式获取任务列表。当使用user_access_token时, 获取与该用户身份相关的所有任务。当使用tenant_access_token时, 获取以该应用身份通过“创建任务“接口创建的所有任务（并非获取该应用所在租户下所有用户创建的任务）。
//
// 本接口支持通过任务创建时间以及任务的完成状态对任务进行过滤。
//
// doc: https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/task-v1/task/list
// 
func (r *TaskService) ListTask(ctx context.Context, request *ListTaskReq, options ...MethodOptionFunc) (*ListTaskResp, *Response, error) {
if r.cli.mock.mockTaskListTask != nil {
		r.cli.Log(ctx, LogLevelDebug, "[lark] Task#ListTask mock enable")
		return r.cli.mock.mockTaskListTask(ctx, request, options...)
	}
 ...
 NeedUserAccessToken: true,
	...
	resp := new(listTaskResp)

	response, err := r.cli.RawRequest(ctx, req, resp)
	return resp.Data, response, err
}

// MockTaskListTask mock TaskListTask method
func (r *Mock) MockTaskListTask(f func(ctx context.Context, request *ListTaskReq, options ...MethodOptionFunc) (*ListTaskResp, *Response, error)) {
	r.mockTaskListTask = f
}
// UnMockTaskListTask un-mock TaskListTask method
func (r *Mock) UnMockTaskListTask() {
	r.mockTaskListTask = nil
}
...
Items []*ListTaskRespItem `json:"items,omitempty"` // 任务列表
...
Due *ListTaskRespItemDue `json:"due,omitempty"` // 任务的截止时间设置
Origin *ListTaskRespItemOrigin `json:"origin,omitempty"` // 任务关联的第三方平台来源信息
...
Followers []*ListTaskRespItemFollower `json:"followers,omitempty"` // 任务的关注者
Collaborators []*ListTaskRespItemCollaborator `json:"collaborators,omitempty"` // 任务的执行者
...
// ListTaskRespItemCollaborator ...
type ListTaskRespItemCollaborator struct { 
...
// ListTaskRespItemDue ...
type ListTaskRespItemDue struct { 
...
// ListTaskRespItemFollower ...
type ListTaskRespItemFollower struct { 
...
// ListTaskRespItemOrigin ...
type ListTaskRespItemOrigin struct { 
...
Href *ListTaskRespItemOriginHref `json:"href,omitempty"` // 任务关联的来源平台详情页链接
}
...
// ListTaskRespItemOriginHref ...
type ListTaskRespItemOriginHref struct { 
...
Data *ListTaskResp `json:"data,omitempty"` 
