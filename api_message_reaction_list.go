...
// GetMessageReactionList 获取指定消息内的表情回复列表, 支持仅获取特定类型的表情回复。
//
// ## 前提条件
...
type GetMessageReactionListReq struct {
	MessageID    string  `path:"message_id" json:"-"`     // 待查询的消息ID。ID 获取方式: - 调用[发送消息](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/create)接口后, 从响应结果的 `message_id` 参数获取。- 监听[接收消息](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/events/receive)事件, 当触发该事件后可以从事件体内获取消息的 `message_id`。- 调用[获取会话历史消息](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/list)接口, 从响应结果的 `message_id` 参数获取。示例值: "om_8964d1b4*2b31383276113"
	ReactionType *string `query:"reaction_type" json:"-"` // 待查询的表情类型, 支持的枚举值参考[表情文案说明](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message-reaction/emojis-introduce)中的 emoji_type 值。注意: 该参数为可选参数, 不传入该参数时将查询消息内所有的表情回复。示例值: LAUGH
	PageToken    *string `query:"page_token" json:"-"`    // 分页标记, 第一次请求不填, 表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token, 下次遍历可采用该 page_token 获取查询结果示例值: YhljsPiGfUgnVAg9urvRFd-BvSqRL20wMZNAWfa9xXkud6UKCybPuUgQ1vM26dj6
	PageSize     *int64  `query:"page_size" json:"-"`     // 分页大小, 用于限制一次请求返回的数据条目数。默认值: 20示例值: 10 最大值: `50
...
type GetMessageReactionListResp struct {
	Items     []*GetMessageReactionListRespItem `json:"items,omitempty"`      // 表情回复列表。
...
type GetMessageReactionListRespItem struct {
	...
	Operator     *GetMessageReactionListRespItemOperator     `json:"operator,omitempty"`      // 添加表情回复的操作人
	ActionTime   string                                      `json:"action_time,omitempty"`   // 添加消息表情回复的时间。Unix 时间戳, 单位: ms
	ReactionType *GetMessageReactionListRespItemReactionType `json:"reaction_type,omitempty"` // 表情类型
}

// GetMessageReactionListRespItemOperator ...
type GetMessageReactionListRespItemOperator struct {
	OperatorID   string `json:"operator_id,omitempty"`   // 操作人 ID, 具体的取值与 `operator_type` 相关: - 当 `operator_type` 取值 `app` 时返回机器人的应用 ID（app_id）。- 当 `operator_type` 取值 `user` 时返回用户的 ID（ID 类型与查询参数 `user_id_type` 的取值一致）。
