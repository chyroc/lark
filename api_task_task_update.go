...
//
// doc: https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/patch
// new doc: https://open.feishu.cn/document/task-v2/task/patch
...
type UpdateTaskReq struct {
	TaskGuid     string             `path:"task_guid" json:"-"`      // 要更新的任务全局唯一ID示例值: "e297ddff-06ca-4166-b917-4ce57cd3a7a0" 最大长度: `100` 字符
	...
	UpdateFields []string           `json:"update_fields,omitempty"` // 设置需要修改的字段任务标题任务描述任务开始时间任务截止时间任务完成时间任务附属自定义数据任务自定义完成规则任务重复规则任务完成模式是否是里程碑任务自定义字段值示例值: ["summary"]
}
...
type UpdateTaskReqTask struct {
	...
	Mode           *int64                           `json:"mode,omitempty"`            // 任务的完成模式。1 - 会签任务；2 - 或签任务示例值: 2 取值范围: `1` ～ `2`
	IsMilestone    *bool                            `json:"is_milestone,omitempty"`    // 是否是里程碑任务示例值: false
...
type UpdateTaskReqTaskStart struct {
	Timestamp *string `json:"timestamp,omitempty"`  // 开始时间的时间戳, 距1970-01-01 00:00:00的毫秒数。如果开始时间是一个日期, 需要把日期转换成时间戳, 并设置 is_all_day=true示例值: "1675454764000"
...
type UpdateTaskRespTask struct {
	...
	AssigneeRelated []*UpdateTaskRespTaskAssigneeRelated `json:"assignee_related,omitempty"` // 任务执行者相关信息, 如会签任务各执行者完成时间等
}

// UpdateTaskRespTaskAssigneeRelated ...
type UpdateTaskRespTaskAssigneeRelated struct {
