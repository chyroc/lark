...
// new doc: https://open.feishu.cn/document/server-docs/vc-v1/meeting/get
...
type GetVCMeetingReq struct {
	...
	WithParticipants   *bool   `query:"with_participants" json:"-"`    // 是否需要参会人列表示例值: false
...
type GetVCMeetingRespMeeting struct {
	...
	Ability                     *GetVCMeetingRespMeetingAbility       `json:"ability,omitempty"`                       // 会中使用的能力注意: 仅当使用应用身份（tenant_access_token）调用该接口, 且查询参数 with_meeting_ability 取值为 true 时, 该参数会有返回值。
}
