...
type SetVCPermissionMeetingRecordingReq struct {
	...
	ActionType        *int64                                                `json:"action_type,omitempty"`        // 授权或者取消授权, 默认授权示例值: 1可选值有: 授权取消授权
}

// SetVCPermissionMeetingRecordingReqPermissionObject ...
type SetVCPermissionMeetingRecordingReqPermissionObject struct {
	ID         *string `json:"id,omitempty"`         // 授权对象ID示例值: "ou_3ec3f6a28a0d08c45d895276e8e5e19b"
	Type       int64   `json:"type,omitempty"`       // 授权对象类型示例值: 1可选值有: 用户授权（id字段填入用户ID）群组授权（id字段填入群组open_chat_id）租户内授权（id字段不填）公网授权（id字段不填）
	Permission int64   `json:"permission,omitempty"` // 权限示例值: 1可选值有: 查看
}
