...
func (r *MailAPI) UpdateMailGroupPatch(ctx context.Context, request *UpdateMailGroupPatchReq, options ...MethodOptionFunc) (*UpdateMailGroupPatchResp, *Response, error) {
	...
	if err != nil {
		r.cli.logError(ctx, "[lark] Mail#UpdateMailGroupPatch PATCH https://open.feishu.cn/open-apis/mail/v1/mailgroups/:mailgroup_id failed: %s", err)
		return nil, response, err
	} else if resp.Code != 0 {
		r.cli.logError(ctx, "[lark] Mail#UpdateMailGroupPatch PATCH https://open.feishu.cn/open-apis/mail/v1/mailgroups/:mailgroup_id failed, code: %d, msg: %s", resp.Code, resp.Msg)
		return nil, response, NewError("Mail", "UpdateMailGroupPatch", resp.Code, resp.Msg)
	}

	r.cli.logDebug(ctx, "[lark] Mail#UpdateMailGroupPatch request_id: %s, response: %s", response.RequestID, jsonString(resp.Data))
