...
func (r *MailAPI) CreateMailGroupPermissionMember(ctx context.Context, request *CreateMailGroupPermissionMemberReq, options ...MethodOptionFunc) (*CreateMailGroupPermissionMemberResp, *Response, error) {
	if r.cli.mock.mockMailCreateMailGroupPermissionMember != nil {
		r.cli.logDebug(ctx, "[lark] Mail#CreateMailGroupPermissionMember mock enable")
		return r.cli.mock.mockMailCreateMailGroupPermissionMember(ctx, request, options...)
	}

	r.cli.logInfo(ctx, "[lark] Mail#CreateMailGroupPermissionMember call api")
	r.cli.logDebug(ctx, "[lark] Mail#CreateMailGroupPermissionMember request: %s", jsonString(request))
	...
	if err != nil {
		r.cli.logError(ctx, "[lark] Mail#CreateMailGroupPermissionMember POST https://open.feishu.cn/open-apis/mail/v1/mailgroups/:mailgroup_id/permission_members failed: %s", err)
		return nil, response, err
	} else if resp.Code != 0 {
		r.cli.logError(ctx, "[lark] Mail#CreateMailGroupPermissionMember POST https://open.feishu.cn/open-apis/mail/v1/mailgroups/:mailgroup_id/permission_members failed, code: %d, msg: %s", resp.Code, resp.Msg)
		return nil, response, NewError("Mail", "CreateMailGroupPermissionMember", resp.Code, resp.Msg)
	}

	r.cli.logDebug(ctx, "[lark] Mail#CreateMailGroupPermissionMember request_id: %s, response: %s", response.RequestID, jsonString(resp.Data))
