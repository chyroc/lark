...
type DocLocation struct {
	...
	StartIndex int    `json:"startIndex,omitempty"` // 元素起始位置偏移量，从 0 开始，范围区间 [0, EOF)
	EndIndex   int    `json:"endIndex,omitempty"`   // 元素结束位置偏移量，范围区间 [0, EOF)
}
...
type DocBlock struct {
	Type           string             `json:"type,omitempty"`           // 由下列类型组成，每个 Block 可以指定一种类型
...
type DocParagraphStyle struct {
	HeadingLevel int           `json:"headingLevel,omitempty"` // 标题级别，支持 1-9 级标题，取值范围：[1,9]
...
type DocParagraphElement struct {
	Type             string               `json:"type,omitempty"`             // 由下列类型组成，每个元素可以指定一种类型
...
type DocReminder struct {
	...
	Timestamp    int          `json:"timestamp,omitempty"`    // 如果设置时间，要求30分或整点；如果不设置IsWholeDay true，时间戳为H:59:59，H和时区相关
	ShouldNotify bool         `json:"shouldNotify,omitempty"` // 是否通知
	NotifyType   int          `json:"notifyType,omitempty"`   // 通知触发时机: isWholeDay 为false时： 0 现在1 提前5分钟2 提前15分钟3 提前30分钟4 提前1小时5 提前2小时6 提前1天7 提前2天 / isWholeDay 为true时：50 当天9点51 提前一天9点52 提前两天9点53 提前一周9点
	Location     *DocLocation `json:"location,omitempty"`     // 元素位置。仅返回，不支持写入
}
...
type DocTable struct {
	...
	RowSize     int              `json:"rowSize,omitempty"`     // 表格行数量，创建空表格时最大值 9
	ColumnSize  int              `json:"columnSize,omitempty"`  // 表格列数量，创建空表格时最大值 9
...
type DocTableRow struct {
	RowIndex   int             `json:"rowIndex,omitempty"`   // 行索引，从 0 开始，第一行是 0，不可编辑
...
type DocTableCell struct {
	...
	ColumnIndex int         `json:"columnIndex,omitempty"` //	列索引，从 0 开始，第一列是 0，不可编辑
	Body        interface{} `json:"body,omitempty"`        // object	单元格内容，支持 Table、Sheet、Bitable 以外的 Block
}
...
type DocSheet struct {
	...
	RowSize    int          `json:"rowSize,omitempty"`    // 电子表格列数量。创建空电子表格时使用，最大值为 9
	ColumnSize int          `json:"columnSize,omitempty"` // 电子表格列数量。创建空电子表格时使用，最大值为 9
	Location   *DocLocation `json:"location,omitempty"`   // 元素位置。仅返回，不支持写入
}
