...
type DocBlock struct {
	...
	File           *DocFile           `json:"file"`                     // 文件上传
	ChatGroup      *DocChatGroup      `json:"chatGroup"`                // 群名片
...
type DocReminder struct {
	...
	NotifyType   int          `json:"notifyType"`   //	通知触发时机isWholeDay 为false时： 0 现在1 提前5分钟2 提前15分钟3 提前30分钟4 提前1小时5 提前2小时6 提前1天7 提前2天isWholeDay 为true时：50 当天9点51 提前一天9点52 提前两天9点53 提前一周9点
	Location     *DocLocation `json:"location"`     //	元素位置。仅返回，不支持写入
}
...
type DocSheet struct {
	Token      string       `json:"token"`      // sheet token，比如shtcnKjLimyn2fW0uGseasQYGgh_m7fMrN 创建接口使用，如果非空，进行深拷贝。不可编辑
