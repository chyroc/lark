...
type OpenWebAppReq struct {
	...
	LkTargetURL *string `json:"lk_target_url,omitempty"` // 指定要打开网页应用的某个页面完整url   注意: 1.此url参数中的scheme和domain必须和网页应用在开发者后台配置的url的scheme和domain一致   2.此url参数必须进行encode处理, 如果url携带query参数, 并且query参数中也包含 url 或 中文 等非ASCII码字符, 必须先对query参数中的非ASCII码字符进行encode后, 作为参数的值, 然后对url整体进行encode。具体用法详见实例3  3. 该参数从 5.12版本 开始支持, 低版本无法解析此参数, 如果在低版本使用, 将打开开发者后台配置的网页应用首页
