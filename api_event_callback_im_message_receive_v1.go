...
type EventMessageReceive struct {
	Event *EventMessageReceiveEvent `json:"event,omitempty"` // \-
}

type EventMessageReceiveEvent struct {
	Sender  *EventMessageReceiveEventSender  `json:"sender,omitempty"`  // 事件的发送者
	Message *EventMessageReceiveEventMessage `json:"message,omitempty"` // 事件中包含的消息内容
}

type EventMessageReceiveEventSender struct {
	SenderID   *EventMessageReceiveEventSenderSenderID `json:"sender_id,omitempty"`   // 用户 ID
	SenderType string                                  `json:"sender_type,omitempty"` // 消息发送者类型。目前只支持用户发送的消息
}

type EventMessageReceiveEventSenderSenderID struct {
...
type EventMessageReceiveEventMessage struct {
	...
	Mentions    []*EventMessageReceiveEventMessageMention `json:"mentions,omitempty"`     // 被提及用户的信息
}

type EventMessageReceiveEventMessageMention struct {
	Key  string                                    `json:"key,omitempty"`  // mention key
	ID   *EventMessageReceiveEventMessageMentionID `json:"id,omitempty"`   // 用户 ID
	Name string                                    `json:"name,omitempty"` // 用户姓名
}

type EventMessageReceiveEventMessageMentionID struct {
	UnionID string `json:"union_id,omitempty"` // 用户的 union id
	UserID  string `json:"user_id,omitempty"`  // 用户的 user id,**字段权限要求**：,<md-perm href="/ssl:ttdoc/ukTMukTMukTM/uQjN3QjL0YzN04CN2cDN">获取用户 userid</md-perm>
	OpenID  string `json:"open_id,omitempty"`  // 用户的 open id
}
