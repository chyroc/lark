...
// UpdateDepartment 调用该接口更新指定部门的信息, 包括名称、父部门以及负责人等信息。
//
// ## 注意事项
...
type UpdateDepartmentReq struct {
	...
	I18nName               *UpdateDepartmentReqI18nName `json:"i18n_name,omitempty"`                 // 部门名称的国际化配置。注意: 不可包含斜杠（`/`）。- 不能与存量部门名称的国际化配置重复。
	ParentDepartmentID     string                       `json:"parent_department_id,omitempty"`      // 父部门的 ID。部门 ID 获取方式: 如果需要将部门的父部门设置为根部门, 则该参数取值 `0`。- 你可以调用[搜索部门](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/department/search)接口获取所需的部门 ID。示例值: "D067"
	...
	Order                  *string                      `json:"order,omitempty"`                     // 部门的排序, 即部门在其同级部门的展示顺序。取值格式为 String 类型的非负整数, 数值越小, 排序越靠前。注意: order 值唯一, 即传入的值不能与存量部门的 order 值重复。- 不传值表示不修改部门原有的 order 值。示例值: "100"
	CreateGroupChat        *bool                        `json:"create_group_chat,omitempty"`         // 是否创建部门群。      可选值有: true: 创建。- false: 不创建。说明: 如果部门之前没有部门群, 则取值为 true 会创建部门群, 创建时, 群名默认为部门名, 群主默认为部门主负责人。- 如果部门之前已创建了部门群, 则该参数无论是否传值均不会影响原有部门群。示例值: false
	Leaders                []*UpdateDepartmentReqLeader `json:"leaders,omitempty"`                   // 部门负责人信息。注意: 配置该参数时, 必须指定一名主负责人。- 设置多名负责人时, 仅支持将某一负责人设置为主负责人。- 部门主管（leader_user_id）和部门主负责人（leaderType 取值为 1 所对应的 leaderID）取值始终一致。因此: - 如果同时设置了部门主管（leader_user_id）, 则此处设置的部门主负责人必须与部门主管为同一个人。    - 仅修改部门主负责人, 会同步修改部门主管（leader_user_id）。
	GroupChatEmployeeTypes []int64                      `json:"group_chat_employee_types,omitempty"` // 部门群的人员类型限制。人员类型的取值范围如下。该参数支持设置多个类型值, 若有多个, 用英文 `, ` 分隔: 1: 正式员工- 2: 实习生- 3: 外包- 4: 劳务- 5: 顾问该参数支持传入自定义人员类型对应的编号。你可以调用[查询人员类型](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/employee_type_enum/list)接口获取相应编号（enum_value）。说明: 如果部门之前已创建了部门群, 则该参数不传值时, 默认不会修改原有的部门群人员类型限制。示例值: [1]
}
...
type UpdateDepartmentReqLeader struct {
	LeaderType int64  `json:"leaderType,omitempty"` // 负责人类型。示例值: 1可选值有: 主负责人。副负责人。
...
type UpdateDepartmentRespDepartment struct {
	...
	I18nName               *UpdateDepartmentRespDepartmentI18nName `json:"i18n_name,omitempty"`                 // 部门名称的国际化配置。字段权限要求（满足任一）: 获取部门基础信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	...
	Status                 *UpdateDepartmentRespDepartmentStatus   `json:"status,omitempty"`                    // 部门状态。字段权限要求（满足任一）: 获取部门基础信息以应用身份访问通讯录读取通讯录以应用身份读取通讯录
	Leaders                []*UpdateDepartmentRespDepartmentLeader `json:"leaders,omitempty"`                   // 部门负责人信息。
