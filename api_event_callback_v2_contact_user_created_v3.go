...
// EventV2ContactUserCreatedV3 当应用订阅该事件后, 如果有新员工入职（例如, 通过管理后台添加成员、调用创建用户 API）, 则会触发该事件。{使用示例}(url=/api/tools/api_explore/api_explore_config?project=contact&version=v3&resource=user&event=created)
//
// ## 前提条件
...
type EventV2ContactUserCreatedV3 struct {
	Object *EventV2ContactUserCreatedV3Object `json:"object,omitempty"` // 员工信息。
}
...
type EventV2ContactUserCreatedV3Object struct {
	...
	Orders                  []*EventV2ContactUserCreatedV3ObjectOrder      `json:"orders,omitempty"`                      // 用户排序信息。用于标记通讯录下组织架构的人员顺序, 人员可能存在多个部门中, 且有不同的排序。字段权限要求（满足任一）: 以应用身份读取通讯录获取用户组织架构信息以应用身份访问通讯录读取通讯录
	CustomAttrs             []*EventV2ContactUserCreatedV3ObjectCustomAttr `json:"custom_attrs,omitempty"`                // 自定义字段信息。了解自定义字段可参见[自定义字段资源介绍](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/custom_attr/overview)。字段权限要求（满足任一）: 以应用身份读取通讯录获取用户受雇信息以应用身份访问通讯录读取通讯录
...
type EventV2ContactUserCreatedV3ObjectCustomAttrValue struct {
	...
	GenericUser *EventV2ContactUserCreatedV3ObjectCustomAttrValueGenericUser `json:"generic_user,omitempty"` // 字段类型为 `GENERIC_USER` 时, 该参数返回定义的引用人员信息。
}
