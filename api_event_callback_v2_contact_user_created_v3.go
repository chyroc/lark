...
type EventV2ContactUserCreatedV3Object struct {
	...
	Avatar        *EventV2ContactUserCreatedV3ObjectAvatar       `json:"avatar,omitempty"`         // 用户头像信息,**字段权限要求（满足任一）**：, 获取用户基本信息, 以应用身份访问通讯录
	Status        *EventV2ContactUserCreatedV3ObjectStatus       `json:"status,omitempty"`         // 用户状态,**字段权限要求（满足任一）**：, 获取用户雇佣信息, 以应用身份访问通讯录
	...
	Orders        []*EventV2ContactUserCreatedV3ObjectOrder      `json:"orders,omitempty"`         // 用户排序信息,**字段权限要求（满足任一）**：, 获取用户组织架构信息, 以应用身份访问通讯录
	CustomAttrs   []*EventV2ContactUserCreatedV3ObjectCustomAttr `json:"custom_attrs,omitempty"`   // 自定义属性,**字段权限要求（满足任一）**：, 获取用户雇佣信息, 以应用身份访问通讯录
}
