...
// UpdateCoreHRPreHire 更新待入职信息接口, 本接口只是会更新待入职数据, 不会校验数据规则, 推荐使用新接口[【更新待入职信息】](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/corehr-v2/pre_hire/patch)。
//
...
type UpdateCoreHRPreHireReq struct {
	PreHireID        string                                  `path:"pre_hire_id" json:"-"`         // 待入职ID示例值: "1616161616"
	ClientToken      *string                                 `query:"client_token" json:"-"`       // 根据client_token是否一致来判断是否为同一请求示例值: 12454646
	AtsApplicationID *string                                 `json:"ats_application_id,omitempty"` // 招聘投递 ID, 详细信息可以通过招聘的【获取投递信息】接口查询获得（仅部分租户可用）示例值: "4719168654814483759"
	HireDate         *string                                 `json:"hire_date,omitempty"`          // 入职日期示例值: "2020-01-01"
	EmployeeType     *UpdateCoreHRPreHireReqEmployeeType     `json:"employee_type,omitempty"`      // 雇佣类型
	WorkerID         *string                                 `json:"worker_id,omitempty"`          // 人员编号示例值: "1245646"
	EmployeeTypeID   *string                                 `json:"employee_type_id,omitempty"`   // 雇佣类型示例值: "正式"
	PersonID         *string                                 `json:"person_id,omitempty"`          // 引用Person ID示例值: "656464648662"
...
type UpdateCoreHRPreHireReqCostCenterRate struct {
	CostCenterID *string `json:"cost_center_id,omitempty"` // 支持的成本中心id示例值: "6950635856373745165"
