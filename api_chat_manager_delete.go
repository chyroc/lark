...
// DeleteChatManager 指定群组, 删除群组内指定的管理员, 包括用户类型的管理员和机器人类型的管理员。
//
// ## 前提条件
...
type DeleteChatManagerReq struct {
	...
	ManagerIDs   []string `json:"manager_ids,omitempty"`    // 要删除的管理员 ID, ID 类型与查询参数 member_id_type 取值一致。- 如果是用户（member_id_type 取值为 user_id/open_id/union_id）, 推荐使用用户的 open_id, 获取方式可参考文档[如何获取 Open ID](https://open.feishu.cn/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-openid)。- 如果是机器人（member_id_type 取值为 app_id）, 请参考[如何获取应用的 App ID](https://open.feishu.cn/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-app-id)注意: 每次请求最多指定 50 个用户或者 5 个机器人示例值: ["ou_9204a37300b3700d61effaa439f34295"] 最大长度: `50
}
