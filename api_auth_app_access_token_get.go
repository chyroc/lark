...
func (r *AuthService) GetAppAccessToken(ctx context.Context) (*TokenExpire, *Response, error) {
	...
	r.cli.logInfo(ctx, "[lark] Auth#GetAppAccessToken call api")

	val, ttl, err := r.cli.store.Get(ctx, genAppTokenKey(r.cli.isISV, r.cli.appID))
	if err != nil && err != ErrStoreNotFound {
		r.cli.logError(ctx, "[lark] Auth#GetAppAccessToken get token from store failed: %s", err)
	} else if val != "" && ttl > 0 {
	...
	if err != nil {
		r.cli.logError(ctx, "[lark] Auth#GetAppAccessToken POST %s failed: %s", uri, err)
		return nil, response, err
	} else if resp.Code != 0 {
		r.cli.logError(ctx, "[lark] Auth#GetAppAccessToken POST %s failed, code: %d, msg: %s", uri, resp.Code, resp.Msg)
		return nil, response, NewError("Token", "GetAppAccessToken", resp.Code, resp.Msg)
	}

	r.cli.logDebug(ctx, "[lark] Auth#GetAppAccessToken request_id: %s, response: %s", response.RequestID, jsonString(resp))

	err = r.cli.store.Set(ctx, genAppTokenKey(r.cli.isISV, r.cli.appID), resp.AppAccessToken, time.Second*time.Duration(resp.Expire))
	if err != nil {
		r.cli.logError(ctx, "[lark] Auth#GetAppAccessToken set token to store failed: %s", err)
	}
