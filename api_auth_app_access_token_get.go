...
func (r *TokenAPI) GetAppAccessToken(ctx context.Context) (*TokenExpire, *Response, error) {
	if r.cli.mock.mockGetAppAccessToken != nil {
		r.cli.logDebug(ctx, "[lark] Token#GetAppAccessToken mock enable")
		return r.cli.mock.mockGetAppAccessToken(ctx)
	}

	r.cli.logInfo(ctx, "[lark] Token#GetAppAccessToken call api")

	val, ttl, err := r.cli.store.Get(ctx, genAppTokenKey(r.cli.isISV, r.cli.appID))
	if err != nil && err != ErrStoreNotFound {
		r.cli.logError(ctx, "[lark] Token#GetAppAccessToken get token from store failed: %s", err)
	} else if val != "" && ttl > 0 {
	...
	if err != nil {
		r.cli.logError(ctx, "[lark] Token#GetAppAccessToken POST %s failed: %s", uri, err)
		return nil, response, err
	} else if resp.Code != 0 {
		r.cli.logError(ctx, "[lark] Token#GetAppAccessToken POST %s failed, code: %d, msg: %s", uri, resp.Code, resp.Msg)
		return nil, response, NewError("Token", "GetAppAccessToken", resp.Code, resp.Msg)
	}

	r.cli.logDebug(ctx, "[lark] Token#GetAppAccessToken request_id: %s, response: %s", response.RequestID, jsonString(resp))

	err = r.cli.store.Set(ctx, genAppTokenKey(r.cli.isISV, r.cli.appID), resp.AppAccessToken, time.Second*time.Duration(resp.Expire))
	if err != nil {
		r.cli.logError(ctx, "[lark] Token#GetAppAccessToken set token to store failed: %s", err)
	}
