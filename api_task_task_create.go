...
//
// doc: https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/create
// new doc: https://open.feishu.cn/document/task-v2/task/create
...
type CreateTaskReq struct {
	...
	Mode              *int64                           `json:"mode,omitempty"`               // 任务完成模式, 1 - 会签任务; 2 - 或签任务示例值: 2默认值: `2` 取值范围: `1` ～ `2`
	IsMilestone       *bool                            `json:"is_milestone,omitempty"`       // 是否是里程碑任务示例值: false默认值: `false`
	...
	DocxSource        *CreateTaskReqDocxSource         `json:"docx_source,omitempty"`        // 如果希望设置任务来源为文档, 则设置此字段- 和extra互斥, 同时设置时报错- 和origin互斥, 同时设置时报错- 和custom_complete互斥, 同时设置时报错
	PositiveReminders []*CreateTaskReqPositiveReminder `json:"positive_reminders,omitempty"` // 正数协议每日提醒 长度范围: `1` ～ `10`
}
...
type CreateTaskReqCustomField struct {
	Guid              string                                 `json:"guid,omitempty"`                // 自定义字段guid示例值: "73b21903-0041-4796-a11e-f8be919a7063"
...
// CreateTaskReqDocxSource ...
type CreateTaskReqDocxSource struct {
	Token   string `json:"token,omitempty"`    // 任务关联的文档token, 要求: 如果使用tenant_access_token请求, 则请求机器人有文档编辑权限；如果使用user_access_token, 则请求用户有文档的编辑权限示例值: "OvZCdFYVHo5ArFxJKHjcnRbtnKd" 长度范围: `27` ～ `27` 字符- 正则校验: `[a-zA-Z0-9]{27}`
	BlockID string `json:"block_id,omitempty"` // 任务关联的文档block_id, 要求block_id存在于token对应文档中、且block_id没有绑定过其他的任务示例值: "O6wwd22uIoG8acxwxGtbljaUcfc" 长度范围: `27` ～ `27` 字符- 正则校验: `[a-zA-Z0-9]{27}`
}

// CreateTaskReqDue ...
type CreateTaskReqDue struct {
	Timestamp *string `json:"timestamp,omitempty"`  // 截止时间/日期的时间戳, 距1970-01-01 00:00:00 UTC的毫秒数。如果截止时间是一个日期, 需要把日期转换成时间戳, 并设置 is_all_day=true示例值: "1675454764000"
...
type CreateTaskReqOriginHref struct {
	URL   *string `json:"url,omitempty"`   // 来源链接对应的地址, 如填写必须以https://或者http://开头。说明: 如需调整 PC 端链接打开方式, 可在飞书客户端的 设置 > 效率 > 链接打开方式 内调整。示例值: "https://www.example.com" 长度范围: `0` ～ `1024` 字符
	Title *string `json:"title,omitempty"` // 来源链接对应的标题示例值: "反馈一个问题, 需要协助排查" 最大长度: `512` 字符
}
...
// CreateTaskReqPositiveReminder ...
type CreateTaskReqPositiveReminder struct {
...
type CreateTaskReqReminder struct {
	RelativeFireMinute int64 `json:"relative_fire_minute,omitempty"` // 相对于截止时间的提醒时间分钟数, 取值必须大于等于0（调用本接口只能设置截止日期前的提醒时间）。例如30表示截止时间前30分钟提醒；0表示截止时提醒。示例值: 30
}
...
type CreateTaskRespTask struct {
	...
	AssigneeRelated   []*CreateTaskRespTaskAssigneeRelated  `json:"assignee_related,omitempty"`   // 任务执行者相关信息, 如会签任务各执行者完成时间等
	PositiveReminders []*CreateTaskRespTaskPositiveReminder `json:"positive_reminders,omitempty"` // 正数协议任务提醒
}

// CreateTaskRespTaskAssigneeRelated ...
type CreateTaskRespTaskAssigneeRelated struct {
...
// CreateTaskRespTaskPositiveReminder ...
type CreateTaskRespTaskPositiveReminder struct {
