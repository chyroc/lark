...
// SearchCoreHRProbation 搜索试用期信息, 创建试用期后立刻搜索, 可能会存在 5s 左右延迟
//
// doc: https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/corehr-v2/probation/search
// new doc: https://open.feishu.cn/document/corehr-v1/probation/search
...
type SearchCoreHRProbationReq struct {
	...
	ProbationStartDateEnd         *string           `json:"probation_start_date_end,omitempty"`          // 试用期开始日期 - 搜索范围结束, 格式: "YYYY-MM-DD"示例值: "2022-05-20"
	ProbationExpectedEndDateStart *string           `json:"probation_expected_end_date_start,omitempty"` // 试用期预计结束日期 - 搜索范围开始, 需要与搜索范围结束一同使用, 格式: "YYYY-MM-DD"示例值: "2022-06-20"
	ProbationExpectedEndDateEnd   *string           `json:"probation_expected_end_date_end,omitempty"`   // 试用期预计结束日期 - 搜索范围结束, 格式: "YYYY-MM-DD"示例值: "2022-07-20"
	ActualProbationEndDateStart   *string           `json:"actual_probation_end_date_start,omitempty"`   // 试用期实际结束日期 - 搜索范围开始, 需要与搜索范围结束一同使用, 格式: "YYYY-MM-DD"示例值: "2022-08-20"
	ActualProbationEndDateEnd     *string           `json:"actual_probation_end_date_end,omitempty"`     // 试用期实际结束日期 - 搜索范围结束, 格式: "YYYY-MM-DD"示例值: "2022-09-20"
	InitiatingTimeStart           *string           `json:"initiating_time_start,omitempty"`             // 转正发起日期 - 搜索范围开始, 需要与搜索范围结束一同使用, 格式: "YYYY-MM-DD"示例值: "2022-10-20"
...
type SearchCoreHRProbationRespItem struct {
	...
	ProbationStartDate              string                                                    `json:"probation_start_date,omitempty"`               // 试用期开始日期, 格式: "YYYY-MM-DD"
	ProbationExpectedEndDate        string                                                    `json:"probation_expected_end_date,omitempty"`        // 试用期预计结束日期, 格式: "YYYY-MM-DD"
	ActualProbationEndDate          string                                                    `json:"actual_probation_end_date,omitempty"`          // 试用期实际结束日期, 格式: "YYYY-MM-DD"
	InitiatingTime                  string                                                    `json:"initiating_time,omitempty"`                    // 转正发起日期, 格式: "YYYY-MM-DD"
	...
	InitiatorID                     string                                                    `json:"initiator_id,omitempty"`                       // 转正发起人的雇佣 ID, 当系统发起转正时该字段为空, 可通过[【搜索员工信息】](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/corehr-v2/employee/search)接口获取
	...
	CustomFields                    []*SearchCoreHRProbationRespItemCustomField               `json:"custom_fields,omitempty"`                      // 自定义字段字段权限要求（满足任一）: 获取试用期自定义字段信息读写试用期自定义字段信息
	...
	Assessments                     []*SearchCoreHRProbationRespItemAssessment                `json:"assessments,omitempty"`                        // 试用期考核结果列表字段权限要求（满足任一）: 获取试用期考核信息读写试用期考核信息
	ProbationExtendExpectedEndDate  string                                                    `json:"probation_extend_expected_end_date,omitempty"` // 试用期延长后的预计结束日期字段权限要求（满足任一）: 获取试用期结果字段写试用期结果字段
	ExtendedProbationPeriodDuration int64                                                     `json:"extended_probation_period_duration,omitempty"` // 试用期延长时间字段权限要求（满足任一）: 获取试用期结果字段写试用期结果字段
	ExtendedProbationPeriodUnit     *SearchCoreHRProbationRespItemExtendedProbationPeriodUnit `json:"extended_probation_period_unit,omitempty"`     // 试用期延长时间单位字段权限要求（满足任一）: 获取试用期结果字段写试用期结果字段
	ProbationOutcome                *SearchCoreHRProbationRespItemProbationOutcome            `json:"probation_outcome,omitempty"`                  // 试用期结果字段权限要求（满足任一）: 获取试用期结果字段写试用期结果字段
}
...
// SearchCoreHRProbationRespItemExtendedProbationPeriodUnit ...
type SearchCoreHRProbationRespItemExtendedProbationPeriodUnit struct {
	EnumName string                                                             `json:"enum_name,omitempty"` // 枚举值
	Display  []*SearchCoreHRProbationRespItemExtendedProbationPeriodUnitDisplay `json:"display,omitempty"`   // 枚举多语展示
}

// SearchCoreHRProbationRespItemExtendedProbationPeriodUnitDisplay ...
type SearchCoreHRProbationRespItemExtendedProbationPeriodUnitDisplay struct {
	Lang  string `json:"lang,omitempty"`  // 语言编码（IETF BCP 47）
	Value string `json:"value,omitempty"` // 文本内容
}
...
// SearchCoreHRProbationRespItemProbationOutcome ...
type SearchCoreHRProbationRespItemProbationOutcome struct {
	EnumName string                                                  `json:"enum_name,omitempty"` // 枚举值
	Display  []*SearchCoreHRProbationRespItemProbationOutcomeDisplay `json:"display,omitempty"`   // 枚举多语展示
}

// SearchCoreHRProbationRespItemProbationOutcomeDisplay ...
type SearchCoreHRProbationRespItemProbationOutcomeDisplay struct {
	Lang  string `json:"lang,omitempty"`  // 语言编码（IETF BCP 47）
	Value string `json:"value,omitempty"` // 文本内容
}
