...
type GetMessageSpecialFocusUnreadReq struct {
	MemberIDType *IDType   `query:"member_id_type" json:"-"` // 指定请求体中 [id_list] 的ID类型示例值: open_id可选值有: 标识一个用户在某个租户内的身份。同一个用户在租户 A 和租户 B 内的 User ID 是不同的。在同一个租户内, 一个用户的 User ID 在所有应用（包括商店应用）中都保持一致。User ID 主要用于在不同的应用间打通用户数据。[了解更多: 如何获取 User ID？](https://open.feishu.cn/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-user-id)标识一个用户在某个应用开发商下的身份。同一用户在同一开发商下的应用中的 Union ID 是相同的, 在不同开发商下的应用中的 Union ID 是不同的。通过 Union ID, 应用开发商可以把同个用户在多个应用中的身份关联起来。[了解更多: 如何获取 Union ID？](https://open.feishu.cn/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-union-id)标识一个用户在某个应用中的身份。同一个用户在不同应用中的 Open ID 不同。[了解更多: 如何获取 Open ID？](https://open.feishu.cn/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-openid)以app_id来识别成员。[了解更多: 如何获取应用的App ID？](https://open.feishu.cn/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-app-id)默认值: `open_id`
	IDList       []string  `json:"id_list,omitempty"`        // 特别关注ID列表。请根据 [member_id_type] 参数填入`open_id`、`user_id`、`union_id`或`app_id`类型的ID示例值: ["ou_e167f0c694cb1c77bb040857dd963024"]
	ChatMode     *ChatMode `json:"chat_mode,omitempty"`      // 按群模式过滤特别关注注意: 当指定 [chat_mode] 为 [p2p]时, 将返回用户与被特别关注者之间单聊的未读消息数。- 当指定[chat_mode] 为[group]时, 将返回被用户特别关注者在群聊中At用户且用户未读的消息数。示例值: "group"可选值有: 群聊单聊默认值: `group`
}
...
type GetMessageSpecialFocusUnreadRespSpecialFocusUnread struct {
	...
	IDType      IDType `json:"id_type,omitempty"`      // 成员ID类型。根据 [member_id_type] 参数返回`open_id`、`user_id`或`union_id`类型的用户ID；机器人返回`app_id`可选值有: 以user_id来识别用户；需要有获取用户 userID的权限 ([什么是 User ID？](https://open.feishu.cn/document/home/user-identity-introduction/user-id))以union_id来识别用户([什么是 Union ID？](https://open.feishu.cn/document/home/user-identity-introduction/union-id))以open_id来识别用户([什么是 Open ID？](https://open.feishu.cn/document/home/user-identity-introduction/open-id))以app_id来识别成员([什么是App ID？](https://open.feishu.cn/document/ukTMukTMukTM/uYTM5UjL2ETO14iNxkTN/terminology#b047be0c))
